...
**97.31% on MNIST with zero learned parameters.** No gradient descent, no
backpropagation, no floating point at inference. Classification is integer
table lookups, AVX2 byte operations, and add/subtract. The entire model fits
in L2 cache.

Pure C, dependency-free. Achieves **85.81% on Fashion-MNIST** with the same
architecture — a sequential field-theoretic ranking system derived from
Green's theorem on the ternary gradient field.
...
1. **Zero learned parameters.** Every weight is derived from closed-form
   mutual information (IG) or grid search over {K, k, probes}. No optimizer,
...
2. **Near-perfect retrieval.** The inverted-index vote puts the correct
   ...
   produce identical accuracy — a 1200x compression ratio with no information
   loss. The problem is ranking, not retrieval.

3. **97.31% with sequential field ranking.** Gradient divergence (Green's
   theorem), Kalman-adaptive weighting, grid spatial decomposition, and
   Bayesian-CfC sequential candidate processing fix 111 errors — breaking
   through the 96.5% block-encoding ceiling.

4. **87% of compute is memory access.** Vote accumulation (scattered writes
   to 240 KB) dominates the pipeline. Dot products cost ~1%. The bottleneck
   is the memory access pattern, not arithmetic.

5. **Generalizes to Fashion-MNIST.** 85.81% (+2.15pp over baseline) using
   the same architecture. Sequential Bayesian processing with aggressive
   decay is the largest single improvement on Fashion (+1.01pp). The
   gradient divergence weight transfers across datasets.

6. **Sub-2 us operating point.** The bytepacked Bayesian hot map classifies
   at 1.9 us/image (91.83% MNIST) with a 16 MB model — competitive with
   optimized neural net inference on embedded hardware.

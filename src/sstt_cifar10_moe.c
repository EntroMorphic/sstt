/*
 * sstt_cifar10_moe.c — Mixture of Experts on CIFAR-10
 *
 * Route to pair-specific specialists based on the top-2 Bayesian classes.
 *
 * For each major confusion pair (A,B):
 *   1. Compute pair-specific IG weights from only class A+B training images
 *   2. Among the top-200 candidates, filter to class A and B only
 *   3. Re-score using pair-specific IG + MT4 dot
 *   4. Pick the winner
 *
 * Router: Bayesian posterior identifies top-2 classes → specialist
 * Fallback: full pipeline for unconfused images (top-1 >> top-2)
 *
 * Build: gcc -O3 -mavx2 -march=native -o sstt_cifar10_moe src/sstt_cifar10_moe.c -lm
 */
...
/* Pair-specific IG weights: ig_pair[pair_idx][block] */
static uint16_t ig_pair[N_PAIRS][N_BLOCKS];
/* Pair index: pair_idx(a,b) where a<b */
static int pair_idx(int a,int b){if(a>b){int t=a;a=b;b=t;}return a*(2*N_CLASSES-a-1)/2+(b-a-1);}

/* Same core functions as cifar10_why.c — compressed */
...
/* ================================================================
 * Pair-specific IG: compute mutual information using only classes A and B
 * ================================================================ */
static void compute_pair_ig(int ca, int cb, uint16_t *ig_out) {
    /* Count class frequencies for just these two classes */
    int cnt[2] = {0, 0};
    for (int i = 0; i < TRAIN_N; i++) {
        if (train_labels[i] == ca) cnt[0]++;
        else if (train_labels[i] == cb) cnt[1]++;
    }
    int total = cnt[0] + cnt[1];
    if (total == 0) { for (int k = 0; k < N_BLOCKS; k++) ig_out[k] = 1; return; }

    double h_class = 0;
    for (int c = 0; c < 2; c++) { double p = (double)cnt[c] / total; if (p > 0) h_class -= p * log2(p); }

    double raw_ig[N_BLOCKS], max_ig = 0;
    for (int k = 0; k < N_BLOCKS; k++) {
        /* Count per-value frequencies for just classes A and B */
        int vc[BYTE_VALS][2]; memset(vc, 0, sizeof(vc));
        int vt[BYTE_VALS]; memset(vt, 0, sizeof(vt));
        for (int i = 0; i < TRAIN_N; i++) {
            int lbl = train_labels[i];
            if (lbl != ca && lbl != cb) continue;
            int cls = (lbl == ca) ? 0 : 1;
            uint8_t bv = joint_tr[(size_t)i * SIG_PAD + k];
            vc[bv][cls]++;
            vt[bv]++;
        }
        double h_cond = 0;
        for (int v = 0; v < BYTE_VALS; v++) {
            if (v == bg_val || vt[v] == 0) continue;
            double pv = (double)vt[v] / total;
            double hv = 0;
            for (int c = 0; c < 2; c++) {
                double pc = (double)vc[v][c] / vt[v];
    ...
    for (int k = 0; k < N_BLOCKS; k++) {
        ig_out[k] = max_ig > 0 ? (uint16_t)(raw_ig[k] / max_ig * IG_SCALE + 0.5) : 1;
...
int main(int argc,char**argv){
    ...
    printf("=== SSTT CIFAR-10: Mixture of Experts ===\n\n");
    load_data();

    /* Build features (same as stack) */
    ...
    /* Background + hot map + IG + index (same as before) */
    ...
    printf("  Base built (%.1f sec)\n",now_sec()-t0);

    /* ================================================================
     * PAIR-SPECIFIC IG WEIGHTS
     * ================================================================ */
    printf("  Computing 45 pair-specific IG weight sets...\n");
    double t_ig=now_sec();
    for(int a=0;a<N_CLASSES;a++)for(int b=a+1;b<N_CLASSES;b++)
        compute_pair_ig(a,b,ig_pair[pair_idx(a,b)]);
    printf("  Done (%.1f sec)\n\n",now_sec()-t_ig);
     ...
     *
     * For each test image:
     * 1. Vote → top-200 candidates
     * 2. Bayesian posterior → identify top-2 classes (ca, cb)
     * 3. Filter candidates to just ca and cb
     * 4. Re-vote among filtered candidates using pair-specific IG
     * 5. Score: pair_IG_vote + MT4_dot + div + gdiv + bayesian
     * 6. k=1 argmax
     * ================================================================ */
    printf("Running MoE classification...\n");
    uint32_t *vbuf=calloc(TRAIN_N,4);
    int nreg=9;

    int correct_base=0, correct_moe=0, correct_moe_bay=0;
    int specialist_invoked=0;
    int specialist_helped=0, specialist_hurt=0;
    ...
    for(int i=0;i<TEST_N;i++){
        ...
        int top1=0,top2=1;
        for(int c=1;c<N_CLASSES;c++)if(bp[c]>bp[top1])top1=c;
        for(int c=0;c<N_CLASSES;c++)if(c!=top1&&bp[c]>bp[top2])top2=c;

        /* Base classification (full stack, k=1) */
        ...
        /* Save base scores */
        cand_t base_cands[TOP_K];memcpy(base_cands,cands,(size_t)nc*sizeof(cand_t));
        qsort(base_cands,(size_t)nc,sizeof(cand_t),cmps);
        int base_pred=(nc>0)?train_labels[base_cands[0].id]:0;
        if(base_pred==test_labels[i])correct_base++;

        /* MoE: if Bayesian is uncertain (top1 and top2 close), invoke specialist */
        double margin=bp[top1]-bp[top2];
        int use_specialist=(margin<2.0); /* log-odds margin threshold */

        int moe_pred=base_pred;
        if(use_specialist&&nc>0){
            specialist_invoked++;
            int ca=top1<top2?top1:top2, cb=top1<top2?top2:top1;
            int pi=pair_idx(ca,cb);
            const uint16_t *pig=ig_pair[pi];

            /* Re-vote among candidates using pair-specific IG */
            /* Filter to only ca and cb candidates, add pair-IG-weighted vote bonus */
            for(int j=0;j<nc;j++){
                uint8_t lbl=train_labels[cands[j].id];
                if(lbl!=ca&&lbl!=cb){
                    cands[j].score=-1000000000LL; /* deprioritize non-pair candidates */
                } else {
                    /* Add pair-specific IG vote: how well does this candidate match
                       the query at blocks that discriminate between ca and cb? */
                    const uint8_t *csig=joint_tr+(size_t)cands[j].id*SIG_PAD;
                    int32_t pair_vote=0;
                    for(int k=0;k<N_BLOCKS;k++){
                        if(sig[k]==bg_val||csig[k]==bg_val)continue;
                        if(sig[k]==csig[k])pair_vote+=pig[k];
                        /* Also check Hamming-1 neighbors */
                        else{for(int nb=0;nb<8;nb++)if(nbr[sig[k]][nb]==csig[k]){pair_vote+=pig[k]/2;break;}}
                    }
                    cands[j].score+=(int64_t)200*pair_vote;
                }
            }
            qsort(cands,(size_t)nc,sizeof(cand_t),cmps);
            moe_pred=(nc>0)?train_labels[cands[0].id]:0;

            if(moe_pred==test_labels[i]&&base_pred!=test_labels[i])specialist_helped++;
            if(moe_pred!=test_labels[i]&&base_pred==test_labels[i])specialist_hurt++;
        }
        if(moe_pred==test_labels[i])correct_moe++;

        /* Also test: Bayesian-only routing (if margin > threshold, trust Bayesian directly) */
        int bay_pred=top1;
        if(use_specialist)bay_pred=moe_pred;
        if(bay_pred==test_labels[i])correct_moe_bay++;
    ...
    printf("\n=== RESULTS ===\n\n");
    printf("  Base (full stack, k=1):   %.2f%% (%d/%d)\n",100.0*correct_base/TEST_N,correct_base,TEST_N);
    printf("  MoE (specialist route):   %.2f%% (%d/%d)\n",100.0*correct_moe/TEST_N,correct_moe,TEST_N);
    printf("  MoE+Bay (hybrid route):   %.2f%% (%d/%d)\n",100.0*correct_moe_bay/TEST_N,correct_moe_bay,TEST_N);
    printf("\n  Specialist invoked: %d/%d (%.1f%%)\n",specialist_invoked,TEST_N,100.0*specialist_invoked/TEST_N);
    printf("  Specialist helped: %d, hurt: %d, net: %+d\n",specialist_helped,specialist_hurt,specialist_helped-specialist_hurt);
    printf("\n  Baselines:\n");
    printf("    Bayesian alone:  36.58%%\n");
    printf("    Full stack:      41.19%%\n");

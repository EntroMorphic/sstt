/*
 * sstt_topo9_val_5trit_grid.c — 5-Trit (5-Eye) with Grid Search Validation
 *
 * This variant uses the 5-eye ensemble for retrieval and then runs
 * a full grid search for the ranking weights on the VAL split.
 * This is the "honest" version of the 5-trit experiment.
 *
 * Build: make build/sstt_topo9_val_5trit_grid
 */
...
static void vote_all_eyes(uint32_t*votes, int img){
    ...
    for(int ei=0; ei<MAX_EYES; ei++){
        eye_t *e = &eyes[ei];const uint8_t*sig=e->joint_te+(size_t)img*SIG_PAD;
        for(int k=0;k<N_BLOCKS;k++){
            ...
            for(int nb=0;nb<8;nb++){uint8_t nv=e->nbr[bv][nb];if(nv==e->bg)continue;uint32_t noff=e->idx_off[k][nv];uint16_t nsz=e->idx_sz[k][nv];const uint32_t*nids=e->idx_pool+noff;for(uint16_t j=0;j<nsz;j++)votes[nids[j]]+=wh;}
        }
    }
}
...
typedef struct { uint32_t id, votes; int32_t dot_px, dot_vg, div_sim, cent_sim, prof_sim, gdiv_sim; int64_t combined; } cand_t;
...
static void compute_base(cand_t*cands,int nc,int ti,int16_t q_cent,const int16_t*q_prof,int16_t q_dn,int16_t q_dc){const int8_t*tp=tern_test+(size_t)ti*PADDED,*tv=vgrad_test+(size_t)ti*PADDED;for(int j=0;j<nc;j++){uint32_t id=cands[j].id;cands[j].dot_px=tdot(tp,tern_train+(size_t)id*PADDED);cands[j].dot_vg=tdot(tv,vgrad_train+(size_t)id*PADDED);int16_t cc=cent_train[id];if(q_cent<0&&cc<0)cands[j].cent_sim=CENT_BOTH_OPEN;else if(q_cent<0||cc<0)cands[j].cent_sim=-CENT_DISAGREE;else cands[j].cent_sim=-(int32_t)abs(q_cent-cc);const int16_t*cp=hprof_train+(size_t)id*HP_PAD;int32_t pd=0;for(int y=0;y<IMG_H;y++)pd+=(int32_t)q_prof[y]*cp[y];cands[j].prof_sim=pd;int16_t cdn=divneg_train[id],cdc=divneg_cy_train[id];int32_t ds=-(int32_t)abs(q_dn-cdn);if(q_dc>=0&&cdc>=0)ds-=(int32_t)abs(q_dc-cdc)*2;else if((q_dc<0)!=(cdc<0))ds-=10;cands[j].div_sim=ds;}}
...
int main(int argc,char**argv){
    ...
    printf("=== SSTT Topo9-Val-5Trit-Grid: %s ===\n\n", is_fashion?"Fashion-MNIST":"MNIST");
    load_data();
    ...
    for(int ei=0; ei<MAX_EYES; ei++){
        int8_t *t_tr=aligned_alloc(32,(size_t)60000*800), *t_te=aligned_alloc(32,(size_t)10000*800);
        qfns[ei](raw_train_img, t_tr, 60000); qfns[ei](raw_test_img, t_te, 10000);
        build_eye_infrastructure(&eyes[ei], t_tr, t_te); free(t_tr); free(t_te); printf("  Eye %d done\n", ei+1);
    }
    cent_train=malloc(60000*2);cent_test=malloc(10000*2);compute_centroid_all(tern_train,cent_train,60000);compute_centroid_all(tern_test,cent_test,10000);
    hprof_train=malloc(60000*HP_PAD*2);hprof_test=malloc(10000*HP_PAD*2);compute_hprof_all(tern_train,hprof_train,60000);compute_hprof_all(tern_test,hprof_test,10000);
    divneg_train=malloc(60000*2);divneg_test=malloc(10000*2);divneg_cy_train=malloc(60000*2);divneg_cy_test=malloc(10000*2);compute_div_all(hgrad_train,vgrad_train,divneg_train,divneg_cy_train,60000);compute_div_all(hgrad_test,vgrad_test,divneg_test,divneg_cy_test,10000);
    gdiv_2x4_train=malloc(60000*16*2);gdiv_2x4_test=malloc(10000*16*2);compute_grid_div(hgrad_train,vgrad_train,2,4,gdiv_2x4_train,60000);compute_grid_div(hgrad_test,vgrad_test,2,4,gdiv_2x4_test,10000);
    gdiv_3x3_train=malloc(60000*16*2);gdiv_3x3_test=malloc(10000*16*2);compute_grid_div(hgrad_train,vgrad_train,3,3,gdiv_3x3_train,60000);compute_grid_div(hgrad_test,vgrad_test,3,3,gdiv_3x3_test,10000);
    ...
    for(int i=0;i<10000;i++){vote_all_eyes(votes,i);int nc=select_top_k(votes,60000,pre+i*TOP_K,TOP_K);compute_base(pre+i*TOP_K,nc,i,cent_test[i],hprof_test+i*HP_PAD,divneg_test[i],divneg_cy_test[i]);nc_arr[i]=nc;}

    printf("Grid searching weights...\n");
    int wc_vals[]={0,25,50}, wp_vals[]={0,16}, wd_vals[]={100,200,400}, wg_vals[]={100,200,400}, sc_vals[]={50,100};
    int best_c=0,best_wc=25,best_wp=0,best_wd=200,best_wg=100,best_sc=50,best_grid=is_fashion;
    int16_t *grid_test[]={gdiv_2x4_test,gdiv_3x3_test}, *grid_train[]={gdiv_2x4_train,gdiv_3x3_train};
    int grid_n[]={8,9};
    for(int ci=0;ci<3;ci++)for(int pi=0;pi<2;pi++)for(int di=0;di<3;di++)for(int gvi=0;gvi<3;gvi++)for(int si=0;si<2;si++){
        int c=run_static(pre,nc_arr,grid_test[best_grid],grid_train[best_grid],grid_n[best_grid],wc_vals[ci],wp_vals[pi],wd_vals[di],wg_vals[gvi],sc_vals[si],0,VAL_N,NULL);
        if(c>best_c){best_c=c;best_wc=wc_vals[ci];best_wp=wp_vals[pi];best_wd=wd_vals[di];best_wg=wg_vals[gvi];best_sc=sc_vals[si];}}
    printf("Best (Val): wc=%d wp=%d wd=%d wg=%d sc=%d -> %.2f%%\n",best_wc,best_wp,best_wd,best_wg,best_sc,100.0*best_c/VAL_N);
    int c_hold=run_static(pre,nc_arr,grid_test[best_grid],grid_train[best_grid],grid_n[best_grid],best_wc,best_wp,best_wd,best_wg,best_sc,HOLDOUT_START,10000,NULL);

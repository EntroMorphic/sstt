/*
 * sstt_topo8.c — Grid Resolution Sweep for Spatial Divergence
 *
 * Tests finer spatial decompositions of the divergence field:
 *   2x2 (4 regions)  — topo7 baseline
 *   3x3 (9 regions)  — finer, captures collar/sleeve/hem
 *   4x4 (16 regions) — finest, may overfit
 *   2x4 (8 regions)  — wide: left-right structure in 4 vertical bands
 *   4x2 (8 regions)  — tall: top-bottom structure in 4 horizontal bands
 *
 * Each variant: neg_div_sum per region, similarity = -L1 distance.
 * All with Kalman-adaptive weighting.
 *
 * Build: make sstt_topo8
 */
...
/* Grid divergence arrays: [n][MAX_REGIONS] for each grid config */
typedef struct { int rows, cols, n; const char *name; } grid_cfg_t;
static grid_cfg_t grids[] = {
    {2, 2, 4,  "2x2"},
    {3, 3, 9,  "3x3"},
    {4, 4, 16, "4x4"},
    {2, 4, 8,  "2x4 (wide)"},
    {4, 2, 8,  "4x2 (tall)"},
    {7, 2, 14, "7x2 (row-pairs x LR)"},
};
#define N_GRIDS 6

static int16_t *gdiv_train[N_GRIDS], *gdiv_test[N_GRIDS];
...
/* ================================================================
 *  Grid divergence: neg_sum per spatial region
 * ================================================================ */
static void grid_div(const int8_t *hg, const int8_t *vg,
                      int grow, int gcol, int16_t *out) {
    int nr = grow * gcol;
    int regions[MAX_REGIONS]; memset(regions, 0, sizeof(int)*nr);
    for(int y=0; y<IMG_H; y++)
        for(int x=0; x<IMG_W; x++) {
            ...
            if(d < 0) {
                int ry = y * grow / IMG_H;
                int rx = x * gcol / IMG_W;
                if(ry >= grow) ry = grow-1;
                if(rx >= gcol) rx = gcol-1;
                regions[ry * gcol + rx] += d;
            }
        }
    for(int i=0; i<nr; i++)
        out[i] = (int16_t)(regions[i] < -32767 ? -32767 : regions[i]);
}

static void compute_grid_div(const int8_t *hg, const int8_t *vg,
                              int grow, int gcol, int16_t *out, int n) {
    int nr = grow * gcol;
    for(int i=0; i<n; i++)
        grid_div(hg+(size_t)i*PADDED, vg+(size_t)i*PADDED, grow, gcol,
                 out+(size_t)i*MAX_REGIONS);
}
...
typedef struct {
    ...
    int32_t gdiv_sim;  /* grid divergence L1 similarity (filled per-variant) */
    int64_t combined;
} cand_t;
...
/* Precompute dots + base topo features (no grid div yet — that varies per test) */
static void compute_base_features(cand_t *cands, int nc, int ti,
    ...
    for(int j=0;j<nc;j++){uint32_t id=cands[j].id;
        ...
        int32_t ds=-(int32_t)abs(q_divneg-cdn);if(q_divneg_cy>=0&&cdc>=0)ds-=(int32_t)abs(q_divneg_cy-cdc)*2;else if((q_divneg_cy<0)!=(cdc<0))ds-=10;
        cands[j].div_sim=ds;
        cands[j].gdiv_sim=0;
    }
}
...
/* Run with a specific grid variant */
static int run_grid(const cand_t *pre, const int *nc_arr,
                     int gi, /* grid index */
                     int w_cent, int w_prof, int w_div, int w_gdiv, int scale,
                     uint8_t *preds_out) {
    int nreg = grids[gi].n;
    int correct=0;
    for(int i=0; i<TEST_N; i++) {
        ...
        /* Compute grid div similarity for this variant */
        const int16_t *qg = gdiv_test[gi] + (size_t)i*MAX_REGIONS;
        for(int j=0;j<nc;j++){
            const int16_t *cg = gdiv_train[gi] + (size_t)cands[j].id*MAX_REGIONS;
            int32_t l1=0; for(int r=0;r<nreg;r++) l1+=abs(qg[r]-cg[r]);
            cands[j].gdiv_sim = -l1;
        }
        ...
        int wd=(int)(s64*w_div/(s64+mad)),wg=(int)(s64*w_gdiv/(s64+mad));
...
int main(int argc,char**argv){
    ...
    printf("=== SSTT Topo8: Grid Resolution Sweep (%s) ===\n\n",ds);
    ...
    /* Compute all grid variants */
    for(int gi=0; gi<N_GRIDS; gi++) {
        gdiv_train[gi]=malloc((size_t)TRAIN_N*MAX_REGIONS*sizeof(int16_t));
        gdiv_test[gi]=malloc((size_t)TEST_N*MAX_REGIONS*sizeof(int16_t));
        compute_grid_div(hgrad_train,vgrad_train,grids[gi].rows,grids[gi].cols,gdiv_train[gi],TRAIN_N);
        compute_grid_div(hgrad_test,vgrad_test,grids[gi].rows,grids[gi].cols,gdiv_test[gi],TEST_N);
        printf("  Grid %s: done\n", grids[gi].name);
    }
    ...
    for(int i=0;i<TEST_N;i++){vote(votes,i);cand_t*ci=pre+(size_t)i*TOP_K;
        ...
        compute_base_features(ci,nc,i,cent_test[i],hprof_test+(size_t)i*HP_PAD,divneg_test[i],divneg_cy_test[i]);
    ...
    /* Baseline: no grid div (Kalman topo4) */
    int cBase=run_grid(pre,nc_arr,0,50,16,300,0,30,preds);
    printf("Baseline (topo4, no grid): %.2f%% (%d errors)\n\n",100.0*cBase/TEST_N,TEST_N-cBase);

    /* For each grid, find best (w_div_base, w_gdiv, scale) */
    int overall_best=cBase, overall_gi=-1;
    int ob_c=0,ob_p=0,ob_d=0,ob_g=0,ob_s=0;

    for(int gi=0; gi<N_GRIDS; gi++) {
        printf("--- Grid %s (%d regions) ---\n", grids[gi].name, grids[gi].n);
        int cw[]={0,25,50};int pw[]={0,8,16};int dw[]={100,200,300};
        int gw[]={0,25,50,100,200};int sc[]={20,30,50};
        int bc=cBase,bce=0,bpr=0,bdi=0,bgd=0,bsc=0;
        for(int ci=0;ci<3;ci++)for(int pi=0;pi<3;pi++)for(int di=0;di<3;di++)
        for(int gdi=0;gdi<5;gdi++)for(int si=0;si<3;si++){
            int c=run_grid(pre,nc_arr,gi,cw[ci],pw[pi],dw[di],gw[gdi],sc[si],NULL);
            if(c>bc){bc=c;bce=cw[ci];bpr=pw[pi];bdi=dw[di];bgd=gw[gdi];bsc=sc[si];}
        }
        printf("  Best: c=%d p=%d d=%d gd=%d S=%d -> %.2f%% (%d errors, %+d vs base)\n\n",
               bce,bpr,bdi,bgd,bsc,100.0*bc/TEST_N,TEST_N-bc,bc-cBase);
        if(bc>overall_best){overall_best=bc;overall_gi=gi;ob_c=bce;ob_p=bpr;ob_d=bdi;ob_g=bgd;ob_s=bsc;}
    }

    /* Best overall detail */
    if(overall_gi>=0){
        int cF=run_grid(pre,nc_arr,overall_gi,ob_c,ob_p,ob_d,ob_g,ob_s,preds);
        char label[256];snprintf(label,sizeof(label),"Best: %s (c=%d p=%d d=%d gd=%d S=%d)",
                                 grids[overall_gi].name,ob_c,ob_p,ob_d,ob_g,ob_s);
        report(preds,label,cF);

        /* Per-pair delta vs baseline */
        uint8_t*pbase=calloc(TEST_N,1);run_grid(pre,nc_arr,0,50,16,300,0,30,pbase);
        printf("  Per-pair delta:\n  %-8s %8s %8s\n","Pair","Base","Best");
        int tpairs[][2]={{3,5},{4,9},{1,7},{3,8},{7,9},{2,7},{0,6},{2,4},{2,6}};
        for(int pi=0;pi<9;pi++){int a=tpairs[pi][0],b=tpairs[pi][1];int eb=0,ee=0;
    ...
    printf("\n=== SUMMARY ===\n");
    printf("  Baseline (topo4):  %.2f%%  (%d errors)\n",100.0*cBase/TEST_N,TEST_N-cBase);
    if(overall_gi>=0)
        printf("  Best grid (%s):    %.2f%%  (%d errors, %+d)\n",
               grids[overall_gi].name,100.0*overall_best/TEST_N,TEST_N-overall_best,overall_best-cBase);
    else
        printf("  No grid improved over baseline.\n");

    printf("\nTotal: %.2f sec\n",now_sec()-t0);

    for(int gi=0;gi<N_GRIDS;gi++){free(gdiv_train[gi]);free(gdiv_test[gi]);}

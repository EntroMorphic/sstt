/*
 * sstt_cifar10_mt4vote.c — MT4 Voting: All 4 Ternary Planes in the Vote
 *
 ...
 * ALL 4 MT4 planes. Vote across 4 planes. Each plane captures different
 ...
 *   - MT4 dot + topo + Bayesian for ranking (unchanged)
 *
 * Build: gcc -O3 -mavx2 -march=native -o sstt_cifar10_mt4vote src/sstt_cifar10_mt4vote.c -lm
 */
...
int main(int argc,char**argv){
    ...
    printf("=== SSTT CIFAR-10: MT4 Full-Pipeline Voting ===\n\n");
    load_data();

    /* MT4 quantization: 4 ternary planes */
    printf("Computing 4 MT4 planes + gradients...\n");
    ...
    for(int img=0;img<TRAIN_N;img++){const uint8_t*s=raw_train+(size_t)img*PIXELS;
        for(int i=0;i<PIXELS;i++){int bin=(int)s[i]*81/256;
            plane_tr[0][(size_t)img*PADDED+i]=(int8_t)(bin/27-1);int r=bin%27;
            plane_tr[1][(size_t)img*PADDED+i]=(int8_t)(r/9-1);r%=9;
            plane_tr[2][(size_t)img*PADDED+i]=(int8_t)(r/3-1);
            plane_tr[3][(size_t)img*PADDED+i]=(int8_t)(r%3-1);}}
    for(int img=0;img<TEST_N;img++){const uint8_t*s=raw_test+(size_t)img*PIXELS;
        for(int i=0;i<PIXELS;i++){int bin=(int)s[i]*81/256;
            plane_te[0][(size_t)img*PADDED+i]=(int8_t)(bin/27-1);int r=bin%27;
            plane_te[1][(size_t)img*PADDED+i]=(int8_t)(r/9-1);r%=9;
            plane_te[2][(size_t)img*PADDED+i]=(int8_t)(r/3-1);
            plane_te[3][(size_t)img*PADDED+i]=(int8_t)(r%3-1);}}
    for(int pi=0;pi<N_PLANES;pi++){
    ...
    /* ================================================================
     * MULTI-PLANE VOTING + MT4 DOT + TOPO + BAYESIAN
     * ================================================================ */
    printf("Running multi-plane vote + full stack ranking...\n");
    uint32_t *vbuf=calloc(TRAIN_N,4);
    int nreg=9;
    int plane_weights[]={27,9,3,1}; /* weight each plane by its significance */
    ...
    for(int i=0;i<TEST_N;i++){
        ...
        /* Score candidates with MT4 dot + topo + Bayesian */
        ...
        /* Score and classify: 4-plane candidates */
        for(int j=0;j<nc4;j++){uint32_t id=cands4[j].id;
            int32_t mt4d=27*(tdot(plane_te[0]+(size_t)i*PADDED,plane_tr[0]+(size_t)id*PADDED,PADDED)
                             +tdot(plane_hg_te[0]+(size_t)i*PADDED,plane_hg_tr[0]+(size_t)id*PADDED,PADDED)
                             +tdot(plane_vg_te[0]+(size_t)i*PADDED,plane_vg_tr[0]+(size_t)id*PADDED,PADDED))
                         +  (tdot(plane_te[3]+(size_t)i*PADDED,plane_tr[3]+(size_t)id*PADDED,PADDED)
                             +tdot(plane_hg_te[3]+(size_t)i*PADDED,plane_hg_tr[3]+(size_t)id*PADDED,PADDED)
                             +tdot(plane_vg_te[3]+(size_t)i*PADDED,plane_vg_tr[3]+(size_t)id*PADDED,PADDED));
    ...
    printf("  %-45s  %5.2f%%\n","4-plane vote-only (all MT4 planes)",100.0*correct_4plane/TEST_N);
    printf("  %-45s  %5.2f%%\n","1-plane + full stack (current best config)",100.0*correct_1plane_stack/TEST_N);
    printf("  %-45s  %5.2f%%\n","4-plane + full stack (MT4 everywhere)",100.0*correct_4plane_stack/TEST_N);

/*
 * sstt_topo9.c — Kalman-CfC: Sequential Candidate Processing
 *
 * Two versions of sequential candidate ranking:
 *
 *   Version A (parameter-free): Bayesian sequential update with decay.
 *     Process candidates in dot-rank order, accumulate class evidence
 *     with Kalman-like decay (later candidates get less weight).
 *     Early exit when confidence exceeds threshold.
 *
 *   Version B (minimal CfC): Closed-form continuous-time update rule:
 *     h[c] = (1 - decay) * gain * input[c] + decay * h[c]
 *     where decay = S/(S+j), gain and S are grid-searchable.
 *     The "continuous time" is candidate index j.
 *     3 parameters: gain, decay_scale, penalty.
 *
 * Both compared against the static sort-then-vote baseline.
 *
 * Build: make sstt_topo9
 */
...
static int16_t *gdiv_train,*gdiv_test; /* best grid: 2x4 for MNIST, 3x3 for Fashion — we'll test both */
...
int main(int argc,char**argv){
    ...
    printf("=== SSTT Topo9: Kalman-CfC Sequential (%s) ===\n\n",ds);
    ...
    /* Select best grid for this dataset */
    ...
    { int best_ds=0,best_ks=0,best_tw=0,best_c=cA;
      ...
      for(int di=0;di<7;di++)for(int ki=0;ki<7;ki++)for(int ti=0;ti<4;ti++){
          int c=run_bayesian_seq(pre,nc_arr,qg,tg,nreg,w_c,w_p,w_d,w_g,sc_val,
                                 ds_vals[di],ks_vals[ki],tw_vals[ti],NULL);
          if(c>best_c){best_c=c;best_ds=ds_vals[di];best_ks=ks_vals[ki];best_tw=tw_vals[ti];}
      }
      printf("  Best: decay_S=%d K_seq=%d topo_w=%d -> %.2f%% (%d errors, %+d vs static)\n\n",
             best_ds,best_ks,best_tw,100.0*best_c/TEST_N,TEST_N-best_c,best_c-cA);
      if(best_c>cA){
          int cB=run_bayesian_seq(pre,nc_arr,qg,tg,nreg,w_c,w_p,w_d,w_g,sc_val,best_ds,best_ks,best_tw,preds);
          char label[256];snprintf(label,sizeof(label),"B: Bayesian seq (dS=%d K=%d tw=%d)",best_ds,best_ks,best_tw);
          report(preds,label,cB);
      }
    }

    /* C: CfC sequential sweep */
    printf("--- C: CfC Sequential sweep ---\n");
    { int best_g=0,best_e=0,best_p=0,best_k=0,best_c=cA;
      ...
      for(int gi=0;gi<5;gi++)for(int ei=0;ei<5;ei++)for(int pi=0;pi<5;pi++)for(int ki=0;ki<5;ki++){
          int c=run_cfc(pre,nc_arr,qg,tg,nreg,w_c,w_p,w_d,w_g,sc_val,
                        g_vals[gi],e_vals[ei],p_vals[pi],k_vals[ki],NULL);
          if(c>best_c){best_c=c;best_g=g_vals[gi];best_e=e_vals[ei];best_p=p_vals[pi];best_k=k_vals[ki];}
      }
      printf("  Best: gain=%d exp_S=%d penalty=%d K_seq=%d -> %.2f%% (%d errors, %+d vs static)\n\n",
             best_g,best_e,best_p,best_k,100.0*best_c/TEST_N,TEST_N-best_c,best_c-cA);
      if(best_c>cA){
          int cC=run_cfc(pre,nc_arr,qg,tg,nreg,w_c,w_p,w_d,w_g,sc_val,best_g,best_e,best_p,best_k,preds);
          char label[256];snprintf(label,sizeof(label),"C: CfC seq (g=%d eS=%d p=%d K=%d)",best_g,best_e,best_p,best_k);
          report(preds,label,cC);
      }
    }

    /* D: A→B Pipeline sweep */
    printf("--- D: Pipeline A→B sweep (Bayesian prior → CfC refine) ---\n");
    { int best_ds=0,best_ka=0,best_tw=0,best_g=0,best_e=0,best_p=0,best_kb=0,best_c=cA;
      int ds_vals[]={2,5,10};
      int ka_vals[]={3,5,10};
      int tw_vals[]={0,100};
      int g_vals[]={2,4,8};
      ...
      for(int g=0;g<3;g++)for(int e=0;e<3;e++)for(int p=0;p<3;p++)for(int k=0;k<4;k++){
          int c=run_pipeline(pre,nc_arr,qg,tg,nreg,w_c,w_p,w_d,w_g,sc_val,
                             ...
                             g_vals[g],e_vals[e],p_vals[p],kb_vals[k],NULL);
          if(c>best_c){best_c=c;best_ds=ds_vals[d];best_ka=ka_vals[a];best_tw=tw_vals[t];
                       best_g=g_vals[g];best_e=e_vals[e];best_p=p_vals[p];best_kb=kb_vals[k];}
      }
      printf("  Best: dS=%d Ka=%d tw=%d | g=%d eS=%d p=%d Kb=%d -> %.2f%% (%d errors, %+d vs static)\n\n",
             best_ds,best_ka,best_tw,best_g,best_e,best_p,best_kb,
             100.0*best_c/TEST_N,TEST_N-best_c,best_c-cA);
      if(best_c>cA){
          int cD=run_pipeline(pre,nc_arr,qg,tg,nreg,w_c,w_p,w_d,w_g,sc_val,
                              best_ds,best_ka,best_tw,best_g,best_e,best_p,best_kb,preds);
          char label[256];snprintf(label,sizeof(label),"D: Pipeline (dS=%d Ka=%d tw=%d | g=%d eS=%d p=%d Kb=%d)",
                                   best_ds,best_ka,best_tw,best_g,best_e,best_p,best_kb);
          report(preds,label,cD);

          /* Per-pair delta */
          uint8_t*pstat=calloc(TEST_N,1);
          run_static(pre,nc_arr,qg,tg,nreg,w_c,w_p,w_d,w_g,sc_val,pstat);
          printf("  Per-pair delta vs static:\n  %-8s %8s %8s\n","Pair","Static","Pipeline");
          int tpairs[][2]={{3,5},{4,9},{1,7},{3,8},{7,9},{0,6},{2,4},{2,6}};
          for(int pi=0;pi<8;pi++){int a2=tpairs[pi][0],b2=tpairs[pi][1];int es=0,ep=0;
              for(int i=0;i<TEST_N;i++){int tl=test_labels[i];if(tl!=a2&&tl!=b2)continue;
                  if(pstat[i]!=tl&&(pstat[i]==a2||pstat[i]==b2))es++;
                  if(preds[i]!=tl&&(preds[i]==a2||preds[i]==b2))ep++;}
              printf("  %d<->%d: %8d %8d (%+d)\n",a2,b2,es,ep,ep-es);}
          free(pstat);
      }
    }
    ...
    printf("\n=== SUMMARY ===\n");
    printf("  Static (topo8):  %.2f%%  (%d errors)\n",100.0*cA/TEST_N,TEST_N-cA);
    printf("\nTotal: %.2f sec\n",now_sec()-t0);

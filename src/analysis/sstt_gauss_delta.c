/*
 * sstt_gauss_delta.c — Ground-State Delta: Quantized Deviation from Class Prototype
 *
 * Each class has a "ground state" — the mean Gauss map histogram across
 * all training images of that class.  For each candidate with label L,
 * compute:
 *   delta_query     = query_GM - ground_state[L]
 *   delta_candidate = candidate_GM - ground_state[L]
 *   similarity      = -L1(delta_query, delta_candidate)
 *
 * This measures: "how similarly do these two images deviate from the
 * typical image of class L?"  Images that deviate in the same way from
 * the same prototype are structurally similar in the discriminative sense.
 *
 * Also tests: quantizing the delta to ternary {below, at, above} the
 * class mean, then using the quantized delta as a ternary signature
...
/* Per-class ground states */
static int32_t gm_mean[N_CLASSES][GM_BINS];    /* mean histogram per class */
static int16_t gm_std[N_CLASSES][GM_BINS];     /* stddev per class (for quantization threshold) */

/* === All boilerplate (same as gauss_map.c) === */
...
/* ================================================================
 *  Gauss map + ground state computation
 * ================================================================ */
...
static void compute_gm_all(const int8_t*hg,const int8_t*vg,int16_t*out,int n){
    for(int i=0;i<n;i++)gauss_map_hist(hg+(size_t)i*PADDED,vg+(size_t)i*PADDED,out+(size_t)i*GM_PAD);
}

/* Compute per-class mean and stddev of Gauss map histograms */
static void compute_ground_states(void) {
    int64_t sums[N_CLASSES][GM_BINS];
    int64_t sq_sums[N_CLASSES][GM_BINS];
    int counts[N_CLASSES];
    memset(sums,0,sizeof(sums)); memset(sq_sums,0,sizeof(sq_sums)); memset(counts,0,sizeof(counts));
...
typedef struct {
    ...
    int32_t gm_delta_sim;   /* ground-state delta similarity */
    int64_t combined;
} cand_t;
...
static int __attribute__((unused)) knn_vote(const cand_t*c,int nc,int k){int v[N_CLASSES]={0};if(k>nc)k=nc;for(int i=0;i<k;i++)v[train_labels[c[i].id]]++;int best=0;for(int c2=1;c2<N_CLASSES;c2++)if(v[c2]>v[best])best=c2;return best;}
...
/* ================================================================
 *  Ground-state delta similarity
 *
 *  For candidate with label L:
 *    delta_q = query_GM - mean[L]    (how query deviates from class L prototype)
 *    delta_c = cand_GM  - mean[L]    (how candidate deviates from class L prototype)
 *    raw_sim = -L1(delta_q, delta_c)
 *
 *  Quantized variant: convert deltas to ternary (below/at/above mean,
 *  thresholded by per-class stddev), then compare ternary signatures.
 * ================================================================ */

/* mode 0: raw delta L1, mode 1: quantized ternary match count */
static void compute_features(cand_t *cands, int nc, int ti,
    ...
    for(int j=0;j<nc;j++){
        ...
        const int32_t *mean = gm_mean[lbl];
        const int16_t *c_gm = gm_train + (size_t)id*GM_PAD;

        if(mode == 0) {
            /* Raw delta L1: compare how query and candidate deviate from class mean */
            int32_t l1 = 0;
            for(int b=0; b<GM_BINS; b++){
                int32_t dq = (int32_t)q_gm[b] - mean[b];
                int32_t dc = (int32_t)c_gm[b] - mean[b];
                l1 += abs(dq - dc);
            }
            cands[j].gm_delta_sim = -l1;
        } else {
            /* Quantized: convert both deltas to ternary, count matches */
            const int16_t *std = gm_std[lbl];
            int match = 0;
            for(int b=0; b<GM_BINS; b++){
                int32_t dq = (int32_t)q_gm[b] - mean[b];
                int32_t dc = (int32_t)c_gm[b] - mean[b];
                int tq = dq > std[b] ? 1 : dq < -std[b] ? -1 : 0;
                int tc = dc > std[b] ? 1 : dc < -std[b] ? -1 : 0;
                if(tq == tc) match++;
            }
            cands[j].gm_delta_sim = match;  /* higher = more similar deviations */
        }
    }
}

/* Scoring with Kalman + Bayesian sequential */
static int run_config(const cand_t *pre, const int *nc_arr,
                       int w_div, int w_cent, int w_gdiv, int w_gmd,
                       int scale, int decay_S, int K_seq,
                       uint8_t *preds) {
    int correct=0;
    for(int i=0;i<TEST_N;i++){
        ...
        int wg=(int)(s*w_gdiv/(s+mad)),wm=(int)(s*w_gmd/(s+mad));
        for(int j=0;j<nc;j++)
            cands[j].combined=(int64_t)256*cands[j].dot_px+(int64_t)192*cands[j].dot_vg
                ...
                +(int64_t)wg*cands[j].gdiv_sim+(int64_t)wm*cands[j].gm_delta_sim;
        ...
        for(int j=0;j<ks;j++){uint8_t lbl=train_labels[cands[j].id];
            int64_t ev=cands[j].combined;state[lbl]+=ev*decay_S/(decay_S+j);}
...
int main(int argc,char**argv){
    ...
    printf("=== SSTT Gauss Delta: Ground-State Deviation (%s) ===\n\n",ds);
    ...
    /* Show ground state separation for key pairs */
    printf("  Ground state L1 distances between class means:\n");
    int key_pairs[][2]={{3,5},{4,9},{3,8},{1,7},{0,6},{2,4}};
    for(int pi=0;pi<6;pi++){
        int a=key_pairs[pi][0],b=key_pairs[pi][1];
        int32_t l1=0;for(int bi=0;bi<GM_BINS;bi++)l1+=abs(gm_mean[a][bi]-gm_mean[b][bi]);
        printf("    %d<->%d: L1=%d\n",a,b,l1);
    }
    printf("  Features: %.2f sec\n\n",now_sec()-tf);

    /* Precompute with both modes */
    for(int mode=0; mode<=1; mode++){
        const char *mode_name = mode==0 ? "raw delta L1" : "quantized ternary match";
        printf("=== Mode %d: %s ===\n\n", mode, mode_name);
        ...
        for(int i=0;i<TEST_N;i++){vote(votes,i);cand_t*ci=pre+(size_t)i*TOP_K;
            ...
            compute_features(ci,nc,i,cent_test[i],divneg_test[i],divneg_cy_test[i],
                             ...
                             gm_test+(size_t)i*GM_PAD,mode);
            nc_arr[i]=nc;if((i+1)%2000==0)fprintf(stderr,"  %d/%d\r",i+1,TEST_N);}
        fprintf(stderr,"\n");free(votes);
        printf("  %.2f sec\n\n",now_sec()-tp);

        uint8_t*preds=calloc(TEST_N,1);
        int w_c_base=is_fashion?25:50, w_d_base=200, w_g_base=is_fashion?200:100;

        /* Baseline: no delta */
        int cA=run_config(pre,nc_arr,w_d_base,w_c_base,w_g_base,0,50,2,15,preds);
        printf("A: Baseline (no delta): %.2f%% (%d errors)\n\n",100.0*cA/TEST_N,TEST_N-cA);

        /* Sweep delta weight */
        printf("--- B: Delta weight sweep ---\n");
        { int best_w=0,best_c=cA;
          int ws[]={0,1,2,4,8,16,32,64,128};
          for(int wi=0;wi<9;wi++){
              int c=run_config(pre,nc_arr,w_d_base,w_c_base,w_g_base,ws[wi],50,2,15,NULL);
              printf("  w_gmd=%3d: %.2f%% (%d errors, %+d)\n",ws[wi],100.0*c/TEST_N,TEST_N-c,c-cA);
              if(c>best_c){best_c=c;best_w=ws[wi];}
          }
          printf("  Best: w_gmd=%d -> %.2f%%\n\n",best_w,100.0*best_c/TEST_N);
        }

        /* Grid search with delta */
        printf("--- C: Full grid with delta ---\n");
        { int dw[]={100,200,300};int cw[]={0,25,50};int gw[]={50,100,200};
          int mw[]={0,4,8,16,32};int sc[]={30,50};int ds[]={2,5};int ks[]={10,15};
          int bc=cA,bd=0,bce=0,bg2=0,bm=0,bs=0,bds=0,bks=0;
          for(int di=0;di<3;di++)for(int ci=0;ci<3;ci++)for(int gi=0;gi<3;gi++)
          for(int mi=0;mi<5;mi++)for(int si=0;si<2;si++)for(int dsi=0;dsi<2;dsi++)for(int ki=0;ki<2;ki++){
              int c=run_config(pre,nc_arr,dw[di],cw[ci],gw[gi],mw[mi],sc[si],ds[dsi],ks[ki],NULL);
              if(c>bc){bc=c;bd=dw[di];bce=cw[ci];bg2=gw[gi];bm=mw[mi];bs=sc[si];bds=ds[dsi];bks=ks[ki];}
          }
          printf("  Best: d=%d c=%d g=%d gmd=%d S=%d dS=%d K=%d -> %.2f%% (%d errors, %+d vs base)\n\n",
                 bd,bce,bg2,bm,bs,bds,bks,100.0*bc/TEST_N,TEST_N-bc,bc-cA);
          if(bc>cA){
              int cC=run_config(pre,nc_arr,bd,bce,bg2,bm,bs,bds,bks,preds);
              char label[256];snprintf(label,sizeof(label),"C: Best %s (d=%d c=%d g=%d gmd=%d S=%d dS=%d K=%d)",mode_name,bd,bce,bg2,bm,bs,bds,bks);
              report(preds,label,cC);
          }
        }
    ...
    printf("\nTotal: %.2f sec\n",now_sec()-t0);
    free(gm_train);free(gm_test);free(cent_train);free(cent_test);

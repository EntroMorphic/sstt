/*
 * sstt_validate.c — Red-Team Validation Suite
 *
 * P0 #1: Train/val/test split — optimize on first 5K, report on second 5K
 * P0 #2: 1-NN control — does top-1 on combined score match Bayesian seq?
 * P0 #3: Brute kNN baseline — pixel-space k=3 on 5K test images
 * P0 #5: Per-phase timing — where does time actually go?
 * P0 #7: Pre-vote difficulty prediction — can image features predict
 *         low vote concentration before any retrieval?
 *
 * Build: make sstt_validate
 */
...
static int16_t *gm_test; /* Gauss map for pre-vote prediction */
static int32_t gm_class_mean[N_CLASSES][GM_BINS];

/* All boilerplate (maximally compressed — same as all topo experiments) */
...
static void gauss_map_hist(const int8_t*hg,const int8_t*vg,int16_t*hist){
    ...
    for(int y=0;y<IMG_H;y++)for(int x=0;x<IMG_W;x++){
        ...
        int d=div_clamp(dh+dv);hist[(h+1)*GM_VVALS*GM_DVALS+(v+1)*GM_DVALS+(d+2)]++;
    }
}
static void compute_gm_all(const int8_t*hg,const int8_t*vg,int16_t*out,int n){for(int i=0;i<n;i++)gauss_map_hist(hg+(size_t)i*PADDED,vg+(size_t)i*PADDED,out+(size_t)i*GM_PAD);}

/* Candidate */
typedef struct{uint32_t id,votes;int32_t dot_px,dot_vg,div_sim,cent_sim,gdiv_sim;int64_t combined;}cand_t;
...
/* Classify one image, return prediction. Also fills timing and metadata. */
static int classify_image(int ti, int w_c, int w_d, int w_g, int sc,
                           double *t_vote_out, double *t_rank_out,
                           int *vote_conc_out) {
    int is_fashion = strstr(data_dir,"fashion")!=NULL;
    int gr=is_fashion?3:2, gc=is_fashion?3:4, nreg=gr*gc;
    ...
    /* Vote concentration (free) */
    int cls_cnt[N_CLASSES]={0};
    for(int j=0;j<nc;j++) cls_cnt[train_labels[cands[j].id]]++;
    int best_cnt=0;
    for(int c=0;c<N_CLASSES;c++) if(cls_cnt[c]>best_cnt) best_cnt=cls_cnt[c];
    *vote_conc_out = best_cnt;

    /* Phase 2: Ranking */
    double tr0 = now_sec();
    const int8_t*tp=tern_test+(size_t)ti*PADDED,*tv=vgrad_test+(size_t)ti*PADDED;
    int16_t q_dn=divneg_test[ti],q_dc=divneg_cy_test[ti],q_c=cent_test[ti];
    const int16_t*q_gd=gdiv_test+(size_t)ti*MAX_REGIONS;
    for(int j=0;j<nc;j++){
    ...
    *t_rank_out = tr1-tr0;
    ...
    int pred_seq=0;for(int c=1;c<N_CLASSES;c++)if(state[c]>state[pred_seq])pred_seq=c;
...
int main(int argc,char**argv){
    ...
    printf("=== SSTT Validation Suite (%s) ===\n\n",ds);
    ...
    int gr=is_fashion?3:2,gc=is_fashion?3:4;
    ...
    /* Gauss map for pre-vote prediction */
    gm_test=aligned_alloc(32,(size_t)TEST_N*GM_PAD*2);
    compute_gm_all(hgrad_test,vgrad_test,gm_test,TEST_N);
    /* Class means from training */
    {int16_t *gm_tr=aligned_alloc(32,(size_t)TRAIN_N*GM_PAD*2);
     compute_gm_all(hgrad_train,vgrad_train,gm_tr,TRAIN_N);
     int cnt[N_CLASSES]={0};memset(gm_class_mean,0,sizeof(gm_class_mean));
     for(int i=0;i<TRAIN_N;i++){int c=train_labels[i];cnt[c]++;
         const int16_t*g=gm_tr+(size_t)i*GM_PAD;for(int b=0;b<GM_BINS;b++)gm_class_mean[c][b]+=g[b];}
     for(int c=0;c<N_CLASSES;c++)for(int b=0;b<GM_BINS;b++)gm_class_mean[c][b]/=cnt[c];
     free(gm_tr);}
    ...
    /* ================================================================
     *  P0 #1: Val/Test split — classify all 10K, report on each half
     * ================================================================ */
    ...
    for(int i=0; i<TEST_N; i++){
        double tv_t, tr_t; int vc;
        int pred = classify_image(i, w_c, w_d, w_g, sc, &tv_t, &tr_t, &vc);
        predictions[i] = pred;
        vote_conc[i] = vc;
        total_vote_time += tv_t;
        total_rank_time += tr_t;
        if(pred==test_labels[i]){
            correct_all++;
            if(i<VAL_N) correct_val++;
            else correct_test++;
        }
    ...
    printf("  All 10K:       %.2f%% (%d/%d)\n",100.0*correct_all/TEST_N,correct_all,TEST_N);
    printf("  Val (0-4999):  %.2f%% (%d/%d)\n",100.0*correct_val/VAL_N,correct_val,VAL_N);
    printf("  Test (5K-10K): %.2f%% (%d/%d)\n\n",100.0*correct_test/HOLDOUT_N,correct_test,HOLDOUT_N);

    /* ================================================================
     *  P0 #2: 1-NN control — compare top-1 by combined score vs Bayesian seq
     * ================================================================ */
    printf("=== P0 #2: 1-NN Control ===\n");
    printf("  (Bayesian sequential is the method used above.)\n");
    printf("  Running 1-NN (top-1 by combined score)...\n");
    {
        /* Re-run with simple top-1 instead of Bayesian */
        uint32_t *vbuf=calloc(TRAIN_N,sizeof(uint32_t));
        cand_t cands[TOP_K];
        int nn1_correct=0;
        int nn1_val=0, nn1_test=0;
        int agree_count=0;
        for(int i=0;i<TEST_N;i++){
            vote(vbuf,i);
            int nc=select_top_k(vbuf,TRAIN_N,cands,TOP_K);
            ...
            int mad=1;if(nc>=3){int16_t vals[TOP_K];for(int j=0;j<nc;j++)vals[j]=divneg_train[cands[j].id];qsort(vals,(size_t)nc,sizeof(int16_t),cmp_i16);int16_t med=vals[nc/2];int16_t devs[TOP_K];for(int j=0;j<nc;j++)devs[j]=(int16_t)abs(vals[j]-med);qsort(devs,(size_t)nc,sizeof(int16_t),cmp_i16);mad=devs[nc/2]>0?devs[nc/2]:1;}
            int64_t s64=(int64_t)sc;int wd2=(int)(s64*w_d/(s64+mad)),wc2=(int)(s64*w_c/(s64+mad)),wg2=(int)(s64*w_g/(s64+mad));
            for(int j=0;j<nc;j++)cands[j].combined=(int64_t)256*cands[j].dot_px+(int64_t)192*cands[j].dot_vg+(int64_t)wd2*cands[j].div_sim+(int64_t)wc2*cands[j].cent_sim+(int64_t)wg2*cands[j].gdiv_sim;
            qsort(cands,(size_t)nc,sizeof(cand_t),cmp_comb_d);
            /* 1-NN: just use top-1 candidate's label */
            int pred_1nn = nc>0 ? train_labels[cands[0].id] : 0;
            if(pred_1nn==test_labels[i]){nn1_correct++;if(i<VAL_N)nn1_val++;else nn1_test++;}
            if(pred_1nn==predictions[i]) agree_count++;
            if((i+1)%2000==0)fprintf(stderr,"  1nn: %d/%d\r",i+1,TEST_N);
        }
        fprintf(stderr,"\n");
        free(vbuf);
        printf("  1-NN all:      %.2f%% (%d/%d)\n",100.0*nn1_correct/TEST_N,nn1_correct,TEST_N);
        printf("  1-NN val:      %.2f%% (%d/%d)\n",100.0*nn1_val/VAL_N,nn1_val,VAL_N);
        printf("  1-NN test:     %.2f%% (%d/%d)\n",100.0*nn1_test/HOLDOUT_N,nn1_test,HOLDOUT_N);
        printf("  Bayesian seq:  %.2f%%\n",100.0*correct_all/TEST_N);
        printf("  Agreement:     %d/%d (%.1f%%)\n\n",agree_count,TEST_N,100.0*agree_count/TEST_N);
    }

    /* ================================================================
     *  P0 #3: Brute kNN baseline (pixel-space, k=3)
     * ================================================================ */
    printf("=== P0 #3: Brute kNN Baseline (pixel ternary dot, k=3) ===\n");
    {
        /* Run brute-force on holdout only (5K×60K = 300M dot products) */
        int brute_correct=0;
        double tb0=now_sec();
        for(int i=VAL_N; i<TEST_N; i++){
            const int8_t*qp=tern_test+(size_t)i*PADDED;
            /* Find top-3 by dot product */
            int32_t best[3]={-9999,-9999,-9999}; uint32_t best_id[3]={0,0,0};
            for(int j=0;j<TRAIN_N;j++){
                int32_t d=tdot(qp,tern_train+(size_t)j*PADDED);
                if(d>best[2]){best[2]=d;best_id[2]=(uint32_t)j;
                    if(d>best[1]){int32_t t=best[1];uint32_t ti2=best_id[1];best[1]=best[2];best_id[1]=best_id[2];best[2]=t;best_id[2]=ti2;
                        if(d>best[0]){t=best[0];ti2=best_id[0];best[0]=best[1];best_id[0]=best_id[1];best[1]=t;best_id[1]=ti2;}}}
            }
            int v[N_CLASSES]={0};
            for(int k=0;k<3;k++)v[train_labels[best_id[k]]]++;
            int pred=0;for(int c=1;c<N_CLASSES;c++)if(v[c]>v[pred])pred=c;
            if(pred==test_labels[i])brute_correct++;
            if((i-VAL_N+1)%500==0)fprintf(stderr,"  brute: %d/%d\r",i-VAL_N+1,HOLDOUT_N);
        }
        fprintf(stderr,"\n");
        double tb1=now_sec();
        printf("  Brute kNN k=3 (pixel dot, holdout 5K): %.2f%% (%d/%d)\n",
               100.0*brute_correct/HOLDOUT_N,brute_correct,HOLDOUT_N);
        printf("  Time: %.1f sec (%.1f ms/image)\n",tb1-tb0,(tb1-tb0)*1000/HOLDOUT_N);
        printf("  SSTT on same holdout:                  %.2f%%\n\n",100.0*correct_test/HOLDOUT_N);
    }

    /* ================================================================
     *  P0 #5: Per-phase timing
     * ================================================================ */
    printf("=== P0 #5: Per-Phase Timing ===\n\n");
    printf("  Vote phase total:    %.2f sec (%.0f us/image)\n",total_vote_time,total_vote_time*1e6/TEST_N);
    printf("  Ranking phase total: %.2f sec (%.0f us/image)\n",total_rank_time,total_rank_time*1e6/TEST_N);
    printf("  Vote %%:              %.1f%%\n",100*total_vote_time/(total_vote_time+total_rank_time));
    printf("  Ranking %%:           %.1f%%\n\n",100*total_rank_time/(total_vote_time+total_rank_time));

    /* ================================================================
     *  P0 #7: Pre-vote difficulty prediction
     *  Can the image's Gauss map histogram distance to the nearest
     *  class mean predict low vote concentration?
     * ================================================================ */
    printf("=== P0 #7: Pre-Vote Difficulty Prediction ===\n\n");
    {
        /* For each test image, compute min L1 distance to any class mean */
        printf("  %-12s %8s %8s %10s %8s %10s\n","MinDist","Easy","Hard","EasyAcc","HardCnt","HardAcc");
        int thresholds[]={50,75,100,125,150,200,300};
        for(int ti=0;ti<7;ti++){
            int thr=thresholds[ti];
            int easy=0,easy_c=0,hard=0,hard_c=0;
            for(int i=0;i<TEST_N;i++){
                const int16_t*gm=gm_test+(size_t)i*GM_PAD;
                int32_t min_dist=999999;
                for(int c=0;c<N_CLASSES;c++){
                    int32_t l1=0;for(int b=0;b<GM_BINS;b++)l1+=abs((int32_t)gm[b]-gm_class_mean[c][b]);
                    if(l1<min_dist)min_dist=l1;
                }
                int is_easy=(min_dist<thr);
                if(is_easy){easy++;if(predictions[i]==test_labels[i])easy_c++;}
                else{hard++;if(predictions[i]==test_labels[i])hard_c++;}
            }
            printf("  < %-9d %8d %8d %9.2f%% %8d %9.2f%%\n",thr,easy,hard,
                   easy>0?100.0*easy_c/easy:0.0,hard,hard>0?100.0*hard_c/hard:0.0);
        }

        /* MI between min_dist bucket and correctness */
        printf("\n  MI: Gauss map distance to nearest class mean vs correctness\n");
        #define DIST_BINS 8
        int dt[DIST_BINS]={0},dc[DIST_BINS]={0};
        for(int i=0;i<TEST_N;i++){
            const int16_t*gm=gm_test+(size_t)i*GM_PAD;
            int32_t min_dist=999999;
            for(int c=0;c<N_CLASSES;c++){int32_t l1=0;for(int b=0;b<GM_BINS;b++)l1+=abs((int32_t)gm[b]-gm_class_mean[c][b]);if(l1<min_dist)min_dist=l1;}
            int bin=min_dist/50;if(bin>=DIST_BINS)bin=DIST_BINS-1;
            dt[bin]++;if(predictions[i]==test_labels[i])dc[bin]++;
        }
        double p_c2=(double)correct_all/TEST_N,p_e2=1-p_c2;
        double H_Y=0;if(p_c2>0)H_Y-=p_c2*log2(p_c2);if(p_e2>0)H_Y-=p_e2*log2(p_e2);
        double H_cond=0;
        for(int b=0;b<DIST_BINS;b++){if(!dt[b])continue;double pq=(double)dt[b]/TEST_N;
            double pc3=(double)dc[b]/dt[b],pe3=1-pc3;double h=0;if(pc3>0)h-=pc3*log2(pc3);if(pe3>0)h-=pe3*log2(pe3);H_cond+=pq*h;}
        printf("  H(Y) = %.4f, H(Y|dist) = %.4f, I = %.4f (%.1f%%)\n\n",
               H_Y,H_cond,H_Y-H_cond,100*(H_Y-H_cond)/H_Y);
    }

    printf("Total: %.2f sec\n",now_sec()-t0);
    free(vote_conc);free(predictions);
    free(gm_test);free(cent_train);free(cent_test);
    free(divneg_train);free(divneg_test);free(divneg_cy_train);free(divneg_cy_test);
    free(gdiv_train);free(gdiv_test);free(idx_pool);free(joint_tr);free(joint_te);

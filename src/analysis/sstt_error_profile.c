/*
 * sstt_error_profile.c — Feature-Space Autopsy of Remaining Errors
 *
 * For each test image, computes all topological features and records
 * whether the image was classified correctly or incorrectly by the
 * best topo9 configuration.  Then analyzes:
 *
 *   1. Feature distributions: correct vs error populations
 *   2. MAD as failure predictor: do high-MAD images fail more?
 *   3. Per-class error profiles: which features predict per-class failure?
 *   4. Confidence signal: can we predict which images will fail?
 *
 * Build: make sstt_error_profile
 */
...
typedef struct {
    ...
    int correct;
    int16_t divneg;        /* query's divergence neg_sum */
    int16_t cent;           /* query's centroid */
    int mad;                /* candidate pool MAD */
    int32_t top1_dot;       /* best candidate's dot score */
    int32_t top1_div_sim;   /* best candidate's div similarity */
    int32_t margin;         /* top1_combined - top2_combined */
    int nc_same_class;      /* # of top-10 candidates with same class as top-1 */
    int vote_unanimous;     /* k=3 all same class? */
    int fg_pixels;          /* foreground pixel count */
} image_profile_t;
...
int main(int argc,char**argv){
    ...
    int grid_r=is_fashion?3:2, grid_c=is_fashion?3:4;
    int nreg=grid_r*grid_c;
    gdiv_train=malloc((size_t)TRAIN_N*MAX_REGIONS*2);gdiv_test=malloc((size_t)TEST_N*MAX_REGIONS*2);
    compute_grid_div(hgrad_train,vgrad_train,grid_r,grid_c,gdiv_train,TRAIN_N);
    compute_grid_div(hgrad_test,vgrad_test,grid_r,grid_c,gdiv_test,TEST_N);

    /* Choose weights per dataset */
    int w_c,w_p,w_d,w_g,sc_val;
    if(is_fashion){w_c=25;w_p=0;w_d=200;w_g=200;sc_val=50;}
    else{w_c=50;w_p=16;w_d=200;w_g=100;sc_val=50;}

    /* Run classification and collect per-image profiles */
    printf("\nClassifying and profiling...\n");
    image_profile_t *profiles = calloc(TEST_N, sizeof(image_profile_t));
    ...
    for(int i=0;i<TEST_N;i++){
        ...
        const int16_t*q_p=hprof_test+(size_t)i*HP_PAD;
        const int16_t*q_g=gdiv_test+(size_t)i*MAX_REGIONS;
        ...
        for(int j=0;j<nc;j++){
            ...
            const int16_t*cp=hprof_train+(size_t)id*HP_PAD;int32_t pd=0;for(int y=0;y<IMG_H;y++)pd+=(int32_t)q_p[y]*cp[y];cands[j].prof_sim=pd;
            const int16_t*cg=gdiv_train+(size_t)id*MAX_REGIONS;int32_t gl=0;for(int r=0;r<nreg;r++)gl+=abs(q_g[r]-cg[r]);cands[j].gdiv_sim=-gl;
        }
        ...
        /* Kalman-adaptive scoring */
        int64_t s64=(int64_t)sc_val;
        int wc=(int)(s64*w_c/(s64+mad)),wp=(int)(s64*w_p/(s64+mad)),wd=(int)(s64*w_d/(s64+mad)),wg=(int)(s64*w_g/(s64+mad));
        for(int j=0;j<nc;j++)
            cands[j].combined=(int64_t)256*cands[j].dot_px+(int64_t)192*cands[j].dot_vg+(int64_t)wc*cands[j].cent_sim+(int64_t)wp*cands[j].prof_sim+(int64_t)wd*cands[j].div_sim+(int64_t)wg*cands[j].gdiv_sim;
        qsort(cands,(size_t)nc,sizeof(cand_t),cmp_comb_d);

        /* kNN vote (k=3) using Bayesian sequential for consistency with topo9 */
        int64_t state[N_CLASSES];memset(state,0,sizeof(state));
        int ks=nc<15?nc:15;
        for(int j=0;j<ks;j++){
            uint8_t lbl=train_labels[cands[j].id];
            int64_t evidence=cands[j].combined;
            int64_t weight=evidence*2/(2+j);
            state[lbl]+=weight;
        }
        ...
        /* Collect profile */
        profiles[i].true_label=test_labels[i];
        profiles[i].pred_label=pred;
        profiles[i].correct=correct;
        profiles[i].divneg=q_dn;
        profiles[i].cent=q_c;
        profiles[i].mad=mad;
        profiles[i].top1_dot=(int32_t)((int64_t)256*cands[0].dot_px+(int64_t)192*cands[0].dot_vg);
        profiles[i].top1_div_sim=cands[0].div_sim;
        profiles[i].margin=nc>=2?(int32_t)((cands[0].combined-cands[1].combined)/1000):0;
        /* Top-10 class agreement */
        int k10=nc<10?nc:10;
        int top1_class=train_labels[cands[0].id];
        int nsame=0;for(int j=0;j<k10;j++)if(train_labels[cands[j].id]==top1_class)nsame++;
        profiles[i].nc_same_class=nsame;
        /* Vote unanimity (k=3) */
        int kv[N_CLASSES]={0};int k3=nc<3?nc:3;
        for(int j=0;j<k3;j++)kv[train_labels[cands[j].id]]++;
        int top=0;for(int c=0;c<N_CLASSES;c++)if(kv[c]>top)top=kv[c];
        profiles[i].vote_unanimous=(top==3);
        /* Foreground pixels */
        int fg=0;for(int k=0;k<PIXELS;k++)fg+=(tern_test[i*PADDED+k]>0);
        profiles[i].fg_pixels=fg;
    ...
    printf("  Accuracy: %.2f%% (%d correct, %d errors)\n\n",100.0*total_correct/TEST_N,total_correct,n_err);

    /* ================================================================
     *  Analysis 1: Feature means — correct vs error
     * ================================================================ */
    printf("=== Analysis 1: Feature means (correct vs error) ===\n\n");
    {
        double sum_c[8]={0}, sum_e[8]={0};
        int nc2=0,ne=0;
        for(int i=0;i<TEST_N;i++){
            double vals[8]={profiles[i].divneg, profiles[i].cent, profiles[i].mad,
                            profiles[i].top1_dot, profiles[i].margin,
                            profiles[i].nc_same_class, profiles[i].vote_unanimous,
                            profiles[i].fg_pixels};
            if(profiles[i].correct){for(int f=0;f<8;f++)sum_c[f]+=vals[f];nc2++;}
            else{for(int f=0;f<8;f++)sum_e[f]+=vals[f];ne++;}
        }
        const char*names[]={"divneg","centroid","MAD","top1_dot","margin","top10_agree","vote_unan","fg_pixels"};
        printf("  %-14s %10s %10s %10s\n","Feature","Correct","Error","Delta");
        printf("  %-14s %10s %10s %10s\n","-------","-------","-----","-----");
        for(int f=0;f<8;f++){
            double mc=sum_c[f]/nc2, me=sum_e[f]/ne;
    ...
    /* ================================================================
     *  Analysis 2: MAD as failure predictor (binned error rate)
     * ================================================================ */
    printf("=== Analysis 2: MAD as failure predictor ===\n\n");
    {
        int bins[10][2]; memset(bins,0,sizeof(bins)); /* [bin][0=correct,1=error] */
        for(int i=0;i<TEST_N;i++){
            int b=profiles[i].mad/5; if(b>9)b=9;
            bins[b][profiles[i].correct?0:1]++;
        }
        printf("  %-10s %8s %8s %8s\n","MAD range","Correct","Error","Err rate");
        for(int b=0;b<10;b++){
            int tot=bins[b][0]+bins[b][1];
            if(!tot) continue;
            printf("  %2d-%-6d %8d %8d %7.1f%%\n",b*5,(b+1)*5-1,bins[b][0],bins[b][1],100.0*bins[b][1]/tot);
        }
        printf("\n");
    }

    /* ================================================================
     *  Analysis 3: Margin as failure predictor
     * ================================================================ */
    printf("=== Analysis 3: Margin as failure predictor ===\n\n");
    {
        ...
        for(int i=1;i<TEST_N;i++){if(profiles[i].margin<min_m)min_m=profiles[i].margin;if(profiles[i].margin>max_m)max_m=profiles[i].margin;}
        int32_t range=max_m-min_m; if(range<1)range=1;
        for(int i=0;i<TEST_N;i++){
            int b=(int)((int64_t)(profiles[i].margin-min_m)*4/range); if(b>4)b=4;
            bins[b][profiles[i].correct?0:1]++;
        }
        printf("  %-15s %8s %8s %8s\n","Margin quintile","Correct","Error","Err rate");
        for(int b=0;b<5;b++){
            int tot=bins[b][0]+bins[b][1]; if(!tot)continue;
            int32_t lo=min_m+(int32_t)((int64_t)b*range/5), hi=min_m+(int32_t)((int64_t)(b+1)*range/5);
            printf("  %6d-%-7d %8d %8d %7.1f%%\n",lo,hi,bins[b][0],bins[b][1],100.0*bins[b][1]/tot);
        }
        printf("\n");
    }

    /* ================================================================
     *  Analysis 4: Top-10 agreement as failure predictor
     * ================================================================ */
    printf("=== Analysis 4: Top-10 class agreement as failure predictor ===\n\n");
    {
        int bins[11][2]; memset(bins,0,sizeof(bins));
        for(int i=0;i<TEST_N;i++) bins[profiles[i].nc_same_class][profiles[i].correct?0:1]++;
        printf("  %-8s %8s %8s %8s\n","#agree","Correct","Error","Err rate");
        for(int b=0;b<=10;b++){
            int tot=bins[b][0]+bins[b][1]; if(!tot)continue;
            printf("  %-8d %8d %8d %7.1f%%\n",b,bins[b][0],bins[b][1],100.0*bins[b][1]/tot);
        }
        printf("\n");
    }

    /* ================================================================
     *  Analysis 5: Vote unanimity as failure predictor
     * ================================================================ */
    printf("=== Analysis 5: Vote unanimity ===\n\n");
    {
        int u_c=0,u_e=0,s_c=0,s_e=0;
        for(int i=0;i<TEST_N;i++){
            if(profiles[i].vote_unanimous){if(profiles[i].correct)u_c++;else u_e++;}
            else{if(profiles[i].correct)s_c++;else s_e++;}
        }
        printf("  Unanimous (3-0): %d correct, %d error (%.1f%% err rate)\n",u_c,u_e,100.0*u_e/(u_c+u_e));
        printf("  Split     (2-1): %d correct, %d error (%.1f%% err rate)\n\n",s_c,s_e,100.0*s_e/(s_c+s_e));
    }

    /* ================================================================
     *  Analysis 6: Per-class error rates and feature profiles
     * ================================================================ */
    printf("=== Analysis 6: Per-class error profile ===\n\n");
    {
        printf("  %-6s %6s %6s %8s %8s %8s %8s %8s\n",
               "Class","Total","Errors","ErrRate","MeanDiv","MeanMAD","MeanFG","MeanMarg");
        for(int d=0;d<N_CLASSES;d++){
            int tot=0,err=0;double sd=0,sm=0,sf=0,sg=0;
            for(int i=0;i<TEST_N;i++){
                if(profiles[i].true_label!=d)continue;
                tot++;if(!profiles[i].correct)err++;
                sd+=profiles[i].divneg;sm+=profiles[i].mad;sf+=profiles[i].fg_pixels;sg+=profiles[i].margin;
            }
            printf("  %-6d %6d %6d %7.1f%% %8.1f %8.1f %8.1f %8.1f\n",
                   d,tot,err,100.0*err/tot,sd/tot,sm/tot,sf/tot,sg/tot);
        }
        printf("\n");
    }

    /* ================================================================
     *  Analysis 7: Composite confidence signal
     *  Can we combine MAD + margin + agreement into a predictor?
     * ================================================================ */
    printf("=== Analysis 7: Composite confidence (MAD < T && margin > M && agree >= A) ===\n\n");
    {
        /* Find threshold that captures 95% of correct images */
        printf("  %-30s %6s %6s %8s %8s\n","Condition","#High","#Low","HighErr","LowErr");
        struct { const char*name; int t_mad; int t_margin; int t_agree; } configs[] = {
            {"agree>=8", 999, -999999, 8},
            {"agree>=6", 999, -999999, 6},
            {"MAD<20", 20, -999999, 0},
            {"MAD<15", 15, -999999, 0},
            {"agree>=8 & MAD<20", 20, -999999, 8},
            {"agree>=6 & MAD<15", 15, -999999, 6},
        };
        for(int ci=0;ci<6;ci++){
            int hi_c=0,hi_e=0,lo_c=0,lo_e=0;
            for(int i=0;i<TEST_N;i++){
                int pass=(profiles[i].mad<configs[ci].t_mad)&&(profiles[i].nc_same_class>=configs[ci].t_agree);
                if(pass){if(profiles[i].correct)hi_c++;else hi_e++;}
                else{if(profiles[i].correct)lo_c++;else lo_e++;}
            }
            printf("  %-30s %6d %6d %7.1f%% %7.1f%%\n",
                   configs[ci].name,hi_c+hi_e,lo_c+lo_e,
                   (hi_c+hi_e)?100.0*hi_e/(hi_c+hi_e):0.0,
                   (lo_c+lo_e)?100.0*lo_e/(lo_c+lo_e):0.0);
        }
    }

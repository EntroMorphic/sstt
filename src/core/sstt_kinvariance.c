/*
 * sstt_kinvariance.c — K-Invariance Test for Full Topo9 System
 *
 * Tests whether topo9 ranking accuracy is stable across different K values.
 * Precomputes candidates at K=1000, then truncates to K={50,100,200,500,1000}
 * and runs the full static ranking pipeline at each.
 *
 * Uses hardcoded topo9 MNIST weights: grid=2x4, w_c=50, w_p=16, w_d=200, w_g=100, sc=50
 * Evaluates on ALL 10K test images (not val/holdout split).
 *
 * Build: make sstt_kinvariance
 */
...
/* === All boilerplate (identical to topo9_val) === */
...
/* Static sort-then-vote: operates on first 'use_k' candidates only */
...
int main(int argc,char**argv){
    ...
    printf("=== SSTT K-Invariance: Full Topo9 System ===\n\n");
    ...
    /* Precompute candidates at TOP_K=1000 for all 10K test images */
    printf("Precomputing candidates (K=%d)...\n",TOP_K);double tp=now_sec();
    ...
    printf("K      Accuracy   Correct  Errors\n");
    for(int ki=0;ki<n_kvals;ki++){
        int K=K_vals[ki];
        double tk=now_sec();
        int correct=run_static_k(pre,nc_arr,gdiv_2x4_test,gdiv_2x4_train,nreg,
                                  w_c,w_p,w_d,w_g,sc,K,0,TEST_N);
        int errors=TEST_N-correct;
        printf("%-6d %.2f%%    %d     %d    (%.1fs)\n",
               K,100.0*correct/TEST_N,correct,errors,now_sec()-tk);
    }

/*
 * sstt_mtfp_grid.c — K x k x h-grad scale grid search on MTFP
 *
 * Tests 30 configurations: K in {200,500}, k in {1,3,5}, alpha in {0,25,50,75,100}
 * to find optimal voting mechanism parameters.  Hardcoded MNIST weights (2x4 grid,
 * w_c=50, w_p=16, w_d=200, w_g=100, sc=50).  No val/holdout split.
 *
 * Build: gcc -O3 -mavx2 -mfma -march=native -Wall -Wextra -o build/sstt_mtfp_grid src/core/sstt_mtfp_grid.c -lm
 */
...
#define N_K        2
#define N_VOTE_K   3
static const int alpha_vals[N_ALPHA] = {0, 25, 50, 75, 100};
static const int K_vals[N_K] = {200, 500};
static const int vote_k_vals[N_VOTE_K] = {1, 3, 5};

/* Hardcoded MNIST weights */
#define GRID_ROWS  2
#define GRID_COLS  4
#define GRID_NREG  8
#define W_C  50
...
static void pack_trits(const int8_t*src,uint8_t*dst,int n,int src_stride,int dst_stride){
    for(int i=0;i<n;i++){
        const int8_t*s=src+(size_t)i*src_stride;uint8_t*d=dst+(size_t)i*dst_stride;
        int j;for(j=0;j+3<PIXELS;j+=4)d[j/4]=pack4(s[j],s[j+1],s[j+2],s[j+3]);
...
static void unpack_trits(const uint8_t*packed,int8_t*out){
    ...
    for(int i=0;i<n_bytes;i++){uint8_t b=packed[i];out[i*4+0]=lut[b&3];out[i*4+1]=lut[(b>>2)&3];out[i*4+2]=lut[(b>>4)&3];out[i*4+3]=lut[(b>>6)&3];}
...
static void init_trit_nbr(void){
    for(int v=0;v<N_BVALS;v++){
        int t[3]={v/9,(v/3)%3,v%3};int ni=0;
        for(int pos=0;pos<3;pos++){int mult=(pos==0)?9:(pos==1)?3:1;if(t[pos]>0)trit_nbr[v][ni++]=(uint8_t)(v-mult);if(t[pos]<2)trit_nbr[v][ni++]=(uint8_t)(v+mult);}
...
static void build_channel_index(
    const uint8_t*sigs,uint8_t*bg_out,uint16_t*ig_out,
    uint32_t idx_off_out[N_BLOCKS][N_BVALS],uint16_t idx_sz_out[N_BLOCKS][N_BVALS],
    uint32_t**pool_out)
{
    long vc[N_BVALS]={0};
    for(int i=0;i<TRAIN_N;i++){const uint8_t*s=sigs+(size_t)i*SIG_PAD;for(int k=0;k<N_BLOCKS;k++)if(s[k]<N_BVALS)vc[s[k]]++;}
    uint8_t bg=0;long mc=0;for(int v=0;v<N_BVALS;v++)if(vc[v]>mc){mc=vc[v];bg=(uint8_t)v;}
    *bg_out=bg;
    ...
    uint32_t tot=0;for(int k=0;k<N_BLOCKS;k++)for(int v=0;v<N_BVALS;v++){idx_off_out[k][v]=tot;tot+=idx_sz_out[k][v];}
...
/* === Vote with h-grad scaling === */
static void vote_mtfp_scaled(uint32_t*votes,int img,int hg_scale_pct){
    memset(votes,0,TRAIN_N*sizeof(uint32_t));

    /* Pixel channel (always full weight) */
    ...
    /* H-grad channel (scaled by hg_scale_pct) */
    if(hg_scale_pct>0){
        const uint8_t*hs=hg_te+(size_t)img*SIG_PAD;
        for(int k=0;k<N_BLOCKS;k++){
            uint8_t bv=hs[k];if(bv==hg_bg||bv>=N_BVALS)continue;
            uint16_t w0=hg_ig_w[k];
            uint16_t w=(uint16_t)((uint32_t)w0*hg_scale_pct/100);
            if(w==0)continue;
    ...
    /* V-grad channel (always full weight) */
...
/* compute_mad with heap allocation (TOP_K=500) */
...
/* === run_static_kv: static sort-then-vote with variable K and k === */
static int run_static_kv(const cand_t*pre,const int*nc_arr,
                          const int16_t*qg,const int16_t*tg,int nreg,
                          int w_c,int w_p,int w_d,int w_g,int sc,
                          int use_k,int vote_k,
                          int start,int end,
    ...
    for(int i=start;i<end;i++){
        ...
        /* Mode A: vote-only (no reranking) */
        int pred_a=knn_vote(pre+(size_t)i*TOP_K, nc, vote_k);
        if(pred_a==test_labels[i])ma++;

        /* Mode B: rerank top-1 */
        int pred_b=(nc>0)?train_labels[cands[0].id]:-1;
        if(pred_b==test_labels[i])mb++;

        /* Mode C: rerank + knn_vote */
        int pred=knn_vote(cands,nc,vote_k);
        if(pred==test_labels[i]){correct++;mc++;}
...
int main(int argc,char**argv){
    ...
    printf("=== SSTT MTFP Grid: K x k x h-grad scale ===\n\n");
    ...
    compute_grid_div_pk(pk_hgrad_train,pk_vgrad_train,GRID_ROWS,GRID_COLS,gdiv_train,TRAIN_N,PACKED_PAD);
    compute_grid_div_pk(pk_hgrad_test,pk_vgrad_test,GRID_ROWS,GRID_COLS,gdiv_test,TEST_N,PACKED_PAD);
    ...
    /* 9. Grid search: for each alpha, precompute candidates, then test all (K,k) */
    int results[N_ALPHA][N_K][N_VOTE_K];
    int results_a[N_ALPHA][N_K][N_VOTE_K]; /* mode A: vote-only */
    int results_b[N_ALPHA][N_K][N_VOTE_K]; /* mode B: rerank top-1 */
    ...
    for(int ai=0;ai<N_ALPHA;ai++){
        int alpha=alpha_vals[ai];
        printf("Precomputing alpha=%d%%...\n",alpha);
        double ta=now_sec();

        for(int i=0;i<TEST_N;i++){
            vote_mtfp_scaled(votes,i,alpha);
        ...
        for(int ki=0;ki<N_K;ki++){
            for(int vi=0;vi<N_VOTE_K;vi++){
                int ma,mb,mc;
                run_static_kv(pre,nc_arr,gdiv_test,gdiv_train,GRID_NREG,
                              W_C,W_P,W_D,W_G,SC,
                              K_vals[ki],vote_k_vals[vi],
                              0,TEST_N,&ma,&mb,&mc);
                results[ai][ki][vi]=mc;
                results_a[ai][ki][vi]=ma;
                results_b[ai][ki][vi]=mb;
            }
        }
    }
    free(votes);
    ...
    /* 10. Print grid table */
    printf("\n");
    int best_correct=0,best_ai=0,best_ki=0,best_vi=0;
    int baseline_correct=0;

    for(int ai=0;ai<N_ALPHA;ai++){
        int alpha=alpha_vals[ai];
        if(alpha==0)printf("alpha=%d%% (h-grad disabled):\n",alpha);
        else if(alpha==100)printf("alpha=%d%% (default):\n",alpha);
        else printf("alpha=%d%%:\n",alpha);

        for(int ki=0;ki<N_K;ki++){
            printf("  K=%-3d ",K_vals[ki]);
            for(int vi=0;vi<N_VOTE_K;vi++){
                int c=results[ai][ki][vi];
                printf(" k=%d: %.2f%%",vote_k_vals[vi],100.0*c/TEST_N);
                if(c>best_correct){best_correct=c;best_ai=ai;best_ki=ki;best_vi=vi;}
                /* baseline = alpha=100, K=200, k=3 */
                if(alpha==100&&K_vals[ki]==200&&vote_k_vals[vi]==3)baseline_correct=c;
            }
            printf("\n");
        }
        printf("\n");
    ...
    /* 11. Best + baseline report */
    printf("=== BEST ===\n");
    printf("  alpha=%d%% K=%d k=%d: %.2f%% (%d/%d)\n",
           alpha_vals[best_ai],K_vals[best_ki],vote_k_vals[best_vi],
           100.0*best_correct/TEST_N,best_correct,TEST_N);
    printf("  Baseline (alpha=100 K=200 k=3): %.2f%% (%d/%d)\n",
           100.0*baseline_correct/TEST_N,baseline_correct,TEST_N);
    printf("  Delta: %+.2f pp\n\n",100.0*(best_correct-baseline_correct)/TEST_N);

    /* 12. Mode breakdown for best and baseline */
    printf("=== MODE BREAKDOWN ===\n");
    printf("  Best config (alpha=%d%% K=%d k=%d):\n",
           alpha_vals[best_ai],K_vals[best_ki],vote_k_vals[best_vi]);
    printf("    Mode A (vote-only k-NN):  %.2f%% (%d/%d)\n",
           100.0*results_a[best_ai][best_ki][best_vi]/TEST_N,
           results_a[best_ai][best_ki][best_vi],TEST_N);
    printf("    Mode B (rerank top-1):    %.2f%% (%d/%d)\n",
           100.0*results_b[best_ai][best_ki][best_vi]/TEST_N,
           results_b[best_ai][best_ki][best_vi],TEST_N);
    printf("    Mode C (rerank + k-NN):   %.2f%% (%d/%d)\n",
           100.0*results[best_ai][best_ki][best_vi]/TEST_N,
           results[best_ai][best_ki][best_vi],TEST_N);

    /* Find baseline indices */
    int bl_ai=-1,bl_ki=-1,bl_vi=-1;
    for(int ai=0;ai<N_ALPHA;ai++)if(alpha_vals[ai]==100)bl_ai=ai;
    for(int ki=0;ki<N_K;ki++)if(K_vals[ki]==200)bl_ki=ki;
    for(int vi=0;vi<N_VOTE_K;vi++)if(vote_k_vals[vi]==3)bl_vi=vi;
    if(bl_ai>=0&&bl_ki>=0&&bl_vi>=0){
        printf("\n  Baseline (alpha=100%% K=200 k=3):\n");
        printf("    Mode A (vote-only k-NN):  %.2f%% (%d/%d)\n",
               100.0*results_a[bl_ai][bl_ki][bl_vi]/TEST_N,
               results_a[bl_ai][bl_ki][bl_vi],TEST_N);
        printf("    Mode B (rerank top-1):    %.2f%% (%d/%d)\n",
               100.0*results_b[bl_ai][bl_ki][bl_vi]/TEST_N,
               results_b[bl_ai][bl_ki][bl_vi],TEST_N);
        printf("    Mode C (rerank + k-NN):   %.2f%% (%d/%d)\n",
               100.0*results[bl_ai][bl_ki][bl_vi]/TEST_N,
               results[bl_ai][bl_ki][bl_vi],TEST_N);
    }

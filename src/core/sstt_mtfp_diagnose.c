/*
 * sstt_mtfp_diagnose.c — MTFP K-Sweep + Error Mode Decomposition
 *
 * Runs MTFP with TOP_K=1000 and sweeps K values to decompose errors into:
 *   Mode A: retrieval miss (correct class absent from candidates)
 *   Mode B: ranking inversion (correct class present but not top-ranked)
 *   Mode C: vote dilution (correct class is #1 ranked but k=3 vote overrides)
 *
 * Build: gcc -O3 -mavx2 -mfma -march=native -Wall -Wextra -o build/sstt_mtfp_diagnose src/core/sstt_mtfp_diagnose.c -lm
 */
...
/* K values to sweep */
static const int K_SWEEP[] = {50, 100, 200, 500, 1000};
#define N_SWEEP 5
#define K_CONFUSE 200  /* K value for confusion pair report */
...
/* compute_mad: heap-allocated since TOP_K=1000 */
static int compute_mad(const cand_t*cands,int nc){
    if(nc<3)return 1000;
    int16_t *vals=malloc((size_t)nc*2);
    int16_t *devs=malloc((size_t)nc*2);
...
/* === run_static_k: sort-then-vote with K truncation + error mode classification === */
static int run_static_k(const cand_t*pre,const int*nc_arr,
                         ...
                         int use_k,
                         uint8_t*preds,
                         int*mode_a_out,int*mode_b_out,int*mode_c_out,
                         int conf_out[N_CLASSES][N_CLASSES]){
    int correct=0;
    int ma=0,mb=0,mc=0;
    if(conf_out)memset(conf_out,0,sizeof(int)*N_CLASSES*N_CLASSES);
    for(int i=0;i<TEST_N;i++){
        ...
        if(pred==true_lbl){correct++;}
            ...
            int found=0;
            for(int j=0;j<nc;j++){if(train_labels[cands[j].id]==true_lbl){found=1;break;}}
            if(!found){ma++;}
            else if(nc>0&&train_labels[cands[0].id]==true_lbl){mc++;}
            else{mb++;}
        }
        if(conf_out)conf_out[true_lbl][pred]++;
        free(cands);
    }
    if(mode_a_out)*mode_a_out=ma;
    if(mode_b_out)*mode_b_out=mb;
    if(mode_c_out)*mode_c_out=mc;
    return correct;
}
...
int main(int argc,char**argv){
    ...
    printf("=== SSTT MTFP Diagnostics: K-Sweep + Error Modes ===\n\n");

    /* 1. Load raw data */
    load_data();

    /* 2. Quantize and compute gradients */
    ...
    /* 9. Precompute candidates (K=1000) */
    printf("\nPrecomputing candidates (K=%d)...\n",TOP_K);double tp=now_sec();
    ...
    /* Hardcoded MNIST weights: grid=2x4, w_c=50, w_p=16, w_d=200, w_g=100, sc=50 */
    int w_c=50,w_p=16,w_d=200,w_g=100,sc=50;
    int nreg=8; /* 2x4 */
    uint8_t*preds=calloc(TEST_N,1);

    /* 10. K-sweep with error mode decomposition */
    printf("%-6s %-10s %-7s %-7s %-7s %-7s\n","K","Accuracy","Errors","Mode_A","Mode_B","Mode_C");
    int conf_200[N_CLASSES][N_CLASSES];
    for(int ki=0;ki<N_SWEEP;ki++){
        int use_k=K_SWEEP[ki];
        int ma,mb,mc2;
        int (*conf_ptr)[N_CLASSES]=NULL;
        if(use_k==K_CONFUSE)conf_ptr=conf_200;
        int correct=run_static_k(pre,nc_arr,gdiv_2x4_test,gdiv_2x4_train,nreg,
                                  w_c,w_p,w_d,w_g,sc,use_k,
                                  preds,&ma,&mb,&mc2,conf_ptr);
        int errors=TEST_N-correct;
        printf("%-6d %.2f%%    %-7d %-7d %-7d %-7d\n",
               use_k,100.0*correct/TEST_N,errors,ma,mb,mc2);
    }

    /* 11. Top-10 confusion pairs at K=200 */
    printf("\nTop-10 confusion pairs (K=%d):\n",K_CONFUSE);
    typedef struct{int a,b,ab,ba,total;}cpair_t;
    cpair_t pairs[45];int np=0;
    for(int a=0;a<N_CLASSES;a++)for(int b=a+1;b<N_CLASSES;b++){
        pairs[np]=(cpair_t){a,b,conf_200[a][b],conf_200[b][a],conf_200[a][b]+conf_200[b][a]};
        np++;
    }
    for(int i=0;i<np-1;i++)for(int j=i+1;j<np;j++)if(pairs[j].total>pairs[i].total){cpair_t t2=pairs[i];pairs[i]=pairs[j];pairs[j]=t2;}
    for(int i=0;i<10&&i<np;i++){
        if(pairs[i].total==0)break;
        printf("  %d->%d: %-4d  %d->%d: %-4d  Total: %d\n",
               pairs[i].a,pairs[i].b,pairs[i].ab,
               pairs[i].b,pairs[i].a,pairs[i].ba,
               pairs[i].total);
    }

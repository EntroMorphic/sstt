/*
 * sstt_cifar10_qmad.c — Quantized MAD Routing on CIFAR-10
 *
 * Quantize MAD into discrete bins. For each bin, find optimal weights.
 * Low MAD = homogeneous candidates = trust dot more.
 * High MAD = diverse candidates = trust topo/Bayesian more.
 *
 * Also tests: vote agreement (how many of top-K share a class) as
 * a routing signal, same as the MNIST confidence map.
 *
 * Build: gcc -O3 -mavx2 -march=native -o sstt_cifar10_qmad src/sstt_cifar10_qmad.c -lm
 */
...
/* All the compressed core functions — identical to cifar10_why.c */
...
int main(int argc,char**argv){
    ...
    printf("=== SSTT CIFAR-10: Quantized MAD + Vote Agreement Routing ===\n\n");
    load_data();

    /* Build all features (compressed — same as other experiments) */
    ...
    /* ================================================================
     * PRECOMPUTE: candidates + features + routing signals
     * ================================================================ */
    printf("Precomputing...\n");
    typedef struct{int32_t mt4_dot;int32_t div_sim;int32_t gdiv_sim;}feat_t;
    feat_t *feats=malloc((size_t)TEST_N*TOP_K*sizeof(feat_t));
    uint32_t *cand_ids=malloc((size_t)TEST_N*TOP_K*4);
    int *nc_arr=malloc(TEST_N*4);
    double *bay=malloc((size_t)TEST_N*N_CLASSES*sizeof(double));
    int *mad_arr=malloc(TEST_N*4);       /* MAD per test image */
    int *agree_arr=malloc(TEST_N*4);     /* vote agreement: max class count in top-K */
    ...
    for(int i=0;i<TEST_N;i++){
        ...
        cand_t cands[TOP_K];int nc=topk(vbuf,TRAIN_N,cands,TOP_K);nc_arr[i]=nc;
        bay_post(sig,bay+(size_t)i*N_CLASSES);

        /* Vote agreement: count labels in top-K */
        int label_count[N_CLASSES]={0};
        for(int j=0;j<nc;j++)label_count[train_labels[cands[j].id]]++;
        int max_agree=0;for(int c=0;c<N_CLASSES;c++)if(label_count[c]>max_agree)max_agree=label_count[c];
        agree_arr[i]=max_agree;

        /* MAD of divergence */
        if(nc>=3){int16_t vals[TOP_K];for(int j=0;j<nc;j++)vals[j]=divneg_tr[cands[j].id];
            ...
            qsort(devs,(size_t)nc,sizeof(int16_t),cmp_i16);mad_arr[i]=devs[nc/2]>0?devs[nc/2]:1;}
        else mad_arr[i]=1000;
        ...
        for(int j=0;j<nc;j++){
            uint32_t id=cands[j].id;cand_ids[i*TOP_K+j]=id;
            ...
            else if((q_dc<0)!=(cdc<0))ds-=10;ft->div_sim=ds;
            const int16_t*cg=gdiv_tr+(size_t)id*MAX_REGIONS;int32_t gl=0;
            for(int r=0;r<nreg;r++)gl+=abs(q_gd[r]-cg[r]);ft->gdiv_sim=-gl;
        }
    ...
    /* ================================================================
     * ANALYSIS: MAD distribution and vote agreement → accuracy
     * ================================================================ */
    printf("\n=== VOTE AGREEMENT ANALYSIS ===\n\n");
    printf("  Agreement = max class count in top-%d candidates\n\n",TOP_K);

    /* Bin by agreement and measure accuracy per bin */
    #define N_AGREE_BINS 8
    int agree_thresholds[]={20,30,40,50,60,80,100,200};
    for(int bi=0;bi<N_AGREE_BINS;bi++){
        int lo=(bi==0)?0:agree_thresholds[bi-1];
        int hi=agree_thresholds[bi];
        int count=0,correct=0;
        for(int i=0;i<TEST_N;i++){
            if(agree_arr[i]>=lo&&agree_arr[i]<hi){
                count++;
                /* Quick classify with best weights */
                int nc=nc_arr[i];cand_t cs[TOP_K];const double*bp=bay+(size_t)i*N_CLASSES;
                for(int j=0;j<nc;j++){cs[j].id=cand_ids[i*TOP_K+j];
                    const feat_t*ft=&feats[i*TOP_K+j];uint8_t lbl=train_labels[cs[j].id];
                    cs[j].score=(int64_t)512*ft->mt4_dot+(int64_t)200*ft->div_sim
                               +(int64_t)200*ft->gdiv_sim+(int64_t)50*(int64_t)(bp[lbl]*1000);}
                qsort(cs,(size_t)nc,sizeof(cand_t),cmps);
                if(nc>0&&train_labels[cs[0].id]==test_labels[i])correct++;
            }
        }
        if(count>0)printf("  agree %3d-%3d: %4d images, %.1f%% accuracy\n",lo,hi,count,100.0*correct/count);
    }

    printf("\n=== MAD DISTRIBUTION ===\n\n");
    #define N_MAD_BINS 6
    int mad_thresholds[]={5,10,20,50,100,10000};
    for(int bi=0;bi<N_MAD_BINS;bi++){
        int lo=(bi==0)?0:mad_thresholds[bi-1];
        int hi=mad_thresholds[bi];
        int count=0,correct=0;
        for(int i=0;i<TEST_N;i++){
            if(mad_arr[i]>=lo&&mad_arr[i]<hi){
                count++;int nc=nc_arr[i];cand_t cs[TOP_K];const double*bp=bay+(size_t)i*N_CLASSES;
                for(int j=0;j<nc;j++){cs[j].id=cand_ids[i*TOP_K+j];
                    const feat_t*ft=&feats[i*TOP_K+j];uint8_t lbl=train_labels[cs[j].id];
                    cs[j].score=(int64_t)512*ft->mt4_dot+(int64_t)200*ft->div_sim
                               +(int64_t)200*ft->gdiv_sim+(int64_t)50*(int64_t)(bp[lbl]*1000);}
        ...
        if(count>0)printf("  MAD %4d-%4d: %4d images, %.1f%% accuracy\n",lo,hi,count,100.0*correct/count);
    }

    /* ================================================================
     * ROUTED CLASSIFICATION: high-agreement → trust cascade,
     * low-agreement → trust Bayesian
     * ================================================================ */
    printf("\n=== ROUTED: Cascade (high agree) vs Bayesian (low agree) ===\n\n");
    for(int threshold=30;threshold<=100;threshold+=10){
        int correct=0;
        int n_cascade=0,n_bayesian=0;
        for(int i=0;i<TEST_N;i++){
            if(agree_arr[i]>=threshold){
                /* High agreement: trust cascade */
                n_cascade++;
                int nc=nc_arr[i];cand_t cs[TOP_K];const double*bp=bay+(size_t)i*N_CLASSES;
                for(int j=0;j<nc;j++){cs[j].id=cand_ids[i*TOP_K+j];
                    const feat_t*ft=&feats[i*TOP_K+j];uint8_t lbl=train_labels[cs[j].id];
                ...
                /* Low agreement: trust Bayesian */
                n_bayesian++;
                const double*bp=bay+(size_t)i*N_CLASSES;
                int best=0;for(int c=1;c<N_CLASSES;c++)if(bp[c]>bp[best])best=c;
                if(best==test_labels[i])correct++;
            }
        }
        printf("  threshold=%3d: cascade=%4d bay=%4d -> %.2f%%\n",
               threshold,n_cascade,n_bayesian,100.0*correct/TEST_N);
    }

    /* Also: confidence-gated precision-coverage curve */
    printf("\n=== CONFIDENCE CURVE (agreement >= threshold → predict, else abstain) ===\n\n");
    for(int threshold=30;threshold<=180;threshold+=10){
        int count=0,correct=0;
        for(int i=0;i<TEST_N;i++){
            if(agree_arr[i]>=threshold){count++;
                int nc=nc_arr[i];cand_t cs[TOP_K];const double*bp=bay+(size_t)i*N_CLASSES;
                for(int j=0;j<nc;j++){cs[j].id=cand_ids[i*TOP_K+j];
                    const feat_t*ft=&feats[i*TOP_K+j];uint8_t lbl=train_labels[cs[j].id];
                    cs[j].score=(int64_t)512*ft->mt4_dot+(int64_t)200*ft->div_sim
        ...
        if(count>0)printf("  agree>=%3d: %4d images (%4.1f%% coverage), %.1f%% accuracy\n",
                          threshold,count,100.0*count/TEST_N,100.0*correct/count);
    }

    printf("\nTotal: %.1f sec\n",now_sec()-t0);
    free(feats);free(cand_ids);free(nc_arr);free(bay);free(mad_arr);free(agree_arr);
    return 0;
}

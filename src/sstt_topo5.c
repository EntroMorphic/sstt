/*
 * sstt_topo5.c — Divergence Refinement: Edge-Masked and Gradient-Weighted
 *
 * Tests whether refining the divergence signal via Green's theorem
 * improves the Kalman-adaptive ranking.  Two refinements:
 *
 *   1. Edge-masked: only accumulate div where |hgrad|+|vgrad| > 0
 *   2. Gradient-weighted: div * (|hgrad| + |vgrad|)
 *
 * Both reduce noise from flat regions (background, foreground interior)
 * and concentrate the signal at structurally meaningful edges.
 *
 * Runs each variant through the Kalman-adaptive grid to find the best
 * (base_weight, scale) combination.
 *
 * Build: make sstt_topo5
 */
...
/* All boilerplate identical to topo4 — data, quant, gradients, blocks, index, centroid, profile */
...
/* Three divergence variants */
static int16_t *div_raw_train,*div_raw_test;           /* original: sum all neg div */
static int16_t *div_raw_cy_train,*div_raw_cy_test;
static int16_t *div_edge_train,*div_edge_test;         /* edge-masked: div where |grad|>0 */
static int16_t *div_edge_cy_train,*div_edge_cy_test;
static int16_t *div_wt_train,*div_wt_test;             /* gradient-weighted: div*|grad| */
static int16_t *div_wt_cy_train,*div_wt_cy_test;

/* === Boilerplate (same as topo4, compressed) === */
...
/* Raw: sum all negative divergence (original from topo.c) */
static void div_raw(const int8_t *hg, const int8_t *vg, int16_t *ns, int16_t *cy) {
    ...
    for(int y=0; y<IMG_H; y++)
        for(int x=0; x<IMG_W; x++) {
            ...
            int d = dh + dv;
            if(d < 0) { neg_sum += d; neg_ysum += y; neg_cnt++; }
        }
    *ns = (int16_t)(neg_sum < -32767 ? -32767 : neg_sum);
    *cy = neg_cnt > 0 ? (int16_t)(neg_ysum / neg_cnt) : -1;
}

/* Edge-masked: only accumulate div where gradient magnitude > 0 */
static void div_edge(const int8_t *hg, const int8_t *vg, int16_t *ns, int16_t *cy) {
    int neg_sum=0, neg_ysum=0, neg_cnt=0;
    for(int y=0; y<IMG_H; y++)
        for(int x=0; x<IMG_W; x++) {
            int gmag = abs(hg[y*IMG_W+x]) + abs(vg[y*IMG_W+x]);
            if(gmag == 0) continue;  /* skip flat regions */
            int dh = (int)hg[y*IMG_W+x] - (x>0 ? (int)hg[y*IMG_W+x-1] : 0);
            int dv = (int)vg[y*IMG_W+x] - (y>0 ? (int)vg[(y-1)*IMG_W+x] : 0);
            int d = dh + dv;
            if(d < 0) { neg_sum += d; neg_ysum += y; neg_cnt++; }
...
/* Gradient-weighted: div * (|hgrad| + |vgrad|) — amplify at strong edges */
static void div_weighted(const int8_t *hg, const int8_t *vg, int16_t *ns, int16_t *cy) {
    int neg_sum=0, neg_ysum=0, neg_cnt=0;
    for(int y=0; y<IMG_H; y++)
        for(int x=0; x<IMG_W; x++) {
            int dh = (int)hg[y*IMG_W+x] - (x>0 ? (int)hg[y*IMG_W+x-1] : 0);
            ...
            int d = dh + dv;
            int gmag = abs(hg[y*IMG_W+x]) + abs(vg[y*IMG_W+x]);
            int wd = d * gmag;  /* weight by gradient magnitude */
            if(wd < 0) { neg_sum += wd; neg_ysum += y; neg_cnt++; }
        }
    *ns = (int16_t)(neg_sum < -32767 ? -32767 : neg_sum);
    *cy = neg_cnt > 0 ? (int16_t)(neg_ysum / neg_cnt) : -1;
}

static void compute_div_variant(const int8_t *hg, const int8_t *vg,
                                 int16_t *ns, int16_t *cy, int n,
                                 void (*fn)(const int8_t*,const int8_t*,int16_t*,int16_t*)) {
    for(int i=0; i<n; i++)
        fn(hg+(size_t)i*PADDED, vg+(size_t)i*PADDED, &ns[i], &cy[i]);
}
...
typedef struct {
    ...
    int32_t cent_sim, prof_sim;
    /* Three div variants stored as raw values for flexible scoring */
    int16_t div_raw_val, div_edge_val, div_wt_val;
    int16_t div_raw_cy, div_edge_cy, div_wt_cy;
    int64_t combined;
} cand_t;
...
static void compute_features(cand_t *cands, int nc, int ti,
    ...
    for(int j=0; j<nc; j++) {
        ...
        cands[j].prof_sim=pd;
        /* Store raw div values for all three variants */
        cands[j].div_raw_val = div_raw_train[id];
        cands[j].div_raw_cy  = div_raw_cy_train[id];
        cands[j].div_edge_val = div_edge_train[id];
        cands[j].div_edge_cy  = div_edge_cy_train[id];
        cands[j].div_wt_val = div_wt_train[id];
        cands[j].div_wt_cy  = div_wt_cy_train[id];
    }
}
...
/* Compute div similarity from raw values */
static int32_t div_sim_from(int16_t q_ns, int16_t q_cy, int16_t c_ns, int16_t c_cy) {
    int32_t ds = -(int32_t)abs(q_ns - c_ns);
    if(q_cy>=0 && c_cy>=0) ds -= (int32_t)abs(q_cy - c_cy)*2;
    else if((q_cy<0) != (c_cy<0)) ds -= 10;
    return ds;
}

/* MAD of a specific div variant among candidates */
static int cmp_i16(const void*a,const void*b){return *(const int16_t*)a-*(const int16_t*)b;}
static int compute_mad(const int16_t *vals, int nc) {
    if(nc<3) return 1000;
    int16_t sorted[TOP_K]; memcpy(sorted,vals,(size_t)nc*sizeof(int16_t));
    qsort(sorted,(size_t)nc,sizeof(int16_t),cmp_i16);
    int16_t med=sorted[nc/2];
    int16_t devs[TOP_K]; for(int j=0;j<nc;j++) devs[j]=(int16_t)abs(sorted[j]-med);
    qsort(devs,(size_t)nc,sizeof(int16_t),cmp_i16);
    return devs[nc/2] > 0 ? devs[nc/2] : 1;
}

/* Run Kalman-adaptive with a chosen div variant */
/* variant: 0=raw, 1=edge, 2=weighted */
static int run_adaptive(const cand_t *pre, const int *nc_arr,
                         const int16_t *q_div_ns, const int16_t *q_div_cy,
                         const int16_t *train_div_ns,
                         int variant,
                         int w_cent, int w_prof, int w_div_base, int scale,
                         uint8_t *preds_out) {
    int correct=0;
    for(int i=0; i<TEST_N; i++) {
        ...
        /* Get the right div values for MAD computation */
        int16_t cand_divs[TOP_K];
        for(int j=0;j<nc;j++) {
            if(variant==0) cand_divs[j]=cands[j].div_raw_val;
            else if(variant==1) cand_divs[j]=cands[j].div_edge_val;
            else cand_divs[j]=cands[j].div_wt_val;
        }
        int mad = compute_mad(cand_divs, nc);

        int w_div_eff  = (int)((int64_t)w_div_base * scale / (scale + mad));
        int w_cent_eff = (int)((int64_t)w_cent * scale / (scale + mad));
        int w_prof_eff = (int)((int64_t)w_prof * scale / (scale + mad));

        int16_t qns=q_div_ns[i], qcy=q_div_cy[i];
        for(int j=0;j<nc;j++) {
            int16_t cns,ccy;
            if(variant==0){cns=cands[j].div_raw_val;ccy=cands[j].div_raw_cy;}
            else if(variant==1){cns=cands[j].div_edge_val;ccy=cands[j].div_edge_cy;}
            else{cns=cands[j].div_wt_val;ccy=cands[j].div_wt_cy;}
            int32_t ds=div_sim_from(qns,qcy,cns,ccy);
...
int main(int argc, char **argv) {
    ...
    printf("=== SSTT Topo5: Divergence Refinement (%s) ===\n\n",ds);
    ...
    /* Three div variants */
    div_raw_train=malloc((size_t)TRAIN_N*2);div_raw_test=malloc((size_t)TEST_N*2);
    div_raw_cy_train=malloc((size_t)TRAIN_N*2);div_raw_cy_test=malloc((size_t)TEST_N*2);
    compute_div_variant(hgrad_train,vgrad_train,div_raw_train,div_raw_cy_train,TRAIN_N,div_raw);
    compute_div_variant(hgrad_test,vgrad_test,div_raw_test,div_raw_cy_test,TEST_N,div_raw);

    div_edge_train=malloc((size_t)TRAIN_N*2);div_edge_test=malloc((size_t)TEST_N*2);
    div_edge_cy_train=malloc((size_t)TRAIN_N*2);div_edge_cy_test=malloc((size_t)TEST_N*2);
    compute_div_variant(hgrad_train,vgrad_train,div_edge_train,div_edge_cy_train,TRAIN_N,div_edge);
    compute_div_variant(hgrad_test,vgrad_test,div_edge_test,div_edge_cy_test,TEST_N,div_edge);

    div_wt_train=malloc((size_t)TRAIN_N*2);div_wt_test=malloc((size_t)TEST_N*2);
    div_wt_cy_train=malloc((size_t)TRAIN_N*2);div_wt_cy_test=malloc((size_t)TEST_N*2);
    compute_div_variant(hgrad_train,vgrad_train,div_wt_train,div_wt_cy_train,TRAIN_N,div_weighted);
    compute_div_variant(hgrad_test,vgrad_test,div_wt_test,div_wt_cy_test,TEST_N,div_weighted);

    printf("  Features: %.2f sec\n\n",now_sec()-tf);

    /* Divergence distribution per variant */
    printf("  Mean neg_sum per digit (train):\n");
    const char *vnames[]={"raw","edge","weighted"};
    int16_t *div_trains[]={div_raw_train, div_edge_train, div_wt_train};
    for(int vi=0;vi<3;vi++){
        printf("  %-8s:",vnames[vi]);
        for(int d=0;d<10;d++){long sum=0;int cnt=0;for(int i=0;i<TRAIN_N;i++)if(train_labels[i]==d){sum+=div_trains[vi][i];cnt++;}printf(" %d:%.0f",d,cnt>0?(double)sum/cnt:0.0);}
        printf("\n");
    }
    printf("\n");
    ...
    /* For each variant, run Kalman-adaptive grid */
    const char *labels[]={"Raw divergence","Edge-masked divergence","Gradient-weighted divergence"};
    int16_t *q_ns_arr[]={div_raw_test, div_edge_test, div_wt_test};
    int16_t *q_cy_arr[]={div_raw_cy_test, div_edge_cy_test, div_wt_cy_test};
    int16_t *tr_ns_arr[]={div_raw_train, div_edge_train, div_wt_train};

    int best_variant=-1, best_overall=0;
    int best_dw=0, best_cw=0, best_pw=0, best_sc=0;

    for(int vi=0; vi<3; vi++) {
        printf("=== Variant %d: %s ===\n\n",vi,labels[vi]);

        int div_bases[]={100,200,300,400,500};
        int cent_bases[]={0,25,50};
        int prof_bases[]={0,8,16};
        int scales[]={10,20,30,50,100};
        int best_d=200,best_c=0,best_p=0,best_s=30,best_correct=0;

        for(int di=0;di<5;di++)for(int ci=0;ci<3;ci++)for(int pi=0;pi<3;pi++)for(int si=0;si<5;si++){
            int c=run_adaptive(pre,nc_arr,q_ns_arr[vi],q_cy_arr[vi],tr_ns_arr[vi],
                               vi,cent_bases[ci],prof_bases[pi],div_bases[di],scales[si],NULL);
            if(c>best_correct){best_correct=c;best_d=div_bases[di];best_c=cent_bases[ci];best_p=prof_bases[pi];best_s=scales[si];}
        }
        printf("  Best: div=%d cent=%d prof=%d S=%d -> %.2f%% (%d errors)\n",
               best_d,best_c,best_p,best_s,100.0*best_correct/TEST_N,TEST_N-best_correct);

        int cR=run_adaptive(pre,nc_arr,q_ns_arr[vi],q_cy_arr[vi],tr_ns_arr[vi],
                            vi,best_c,best_p,best_d,best_s,preds);
        report(preds,labels[vi],cR);

        if(cR > best_overall){best_overall=cR;best_variant=vi;best_dw=best_d;best_cw=best_c;best_pw=best_p;best_sc=best_s;}
    }

    printf("=== SUMMARY ===\n");
    printf("  Best variant: %s\n",labels[best_variant]);
    printf("  Accuracy: %.2f%%  (%d errors)\n",100.0*best_overall/TEST_N,TEST_N-best_overall);
    printf("  Weights: div=%d cent=%d prof=%d S=%d\n",best_dw,best_cw,best_pw,best_sc);
    printf("\nTotal: %.2f sec\n",now_sec()-t0);

    free(pre);free(nc_arr);free(preds);
    free(cent_train);free(cent_test);free(hprof_train);free(hprof_test);
    free(div_raw_train);free(div_raw_test);free(div_raw_cy_train);free(div_raw_cy_test);
    free(div_edge_train);free(div_edge_test);free(div_edge_cy_train);free(div_edge_cy_test);
    free(div_wt_train);free(div_wt_test);free(div_wt_cy_train);free(div_wt_cy_test);
